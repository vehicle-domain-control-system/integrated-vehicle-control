# Function Controller System Requirement (SR) — Functional Baseline Draft
...
본 문서는 확정된 실내 기능의 동작을 실제로 수행하고, 그 결과를 확인하여 되돌려 주는 Function Controller의 상위 요구사항을 정의한다.

Function Controller는 동작 여부를 스스로 결정하지 않는다. 확정된 동작을 받아 수행하고, 그 결과를 확인하여 제공한다.

---

## 2. 기능 범위

Function Controller는 다음 기능의 실제 동작과 결과 확인을 담당한다.
...
실내 온도나 주변 밝기와 같은 측정값은 Function Controller에 직접 제공되지 않는다. 측정값과 사용자 요청은 상위 차량 시스템에서 종합되어 동작이 확정되며, Function Controller는 확정된 동작을 수행한다.

Function Controller는 확정된 동작을 실제로 수행하고, 실제 동작 상태를 확인하여 상위 차량 시스템에 제공한다.

Function Controller는 다른 기능 사이의 정보를 중계하지 않는다.
...
- 시스템은 확정된 표시에 따라 실내 조명을 켜야 한다.
- 시스템은 확정되지 않은 조명 요청을 정상적인 표시로 실행하지 않아야 한다.
- 시스템은 현재 켜져 있는 표시와 밝기를 상위 차량 시스템에서 확인할 수 있도록 해야 한다.
...
본 절의 판단은 Function Controller가 직접 측정하는 정보에 근거하며, 통신 상태와 무관하게 수행되어야 한다.
...
**로컬 안전 판단은 전달받은 동작을 수행하지 않을 수 있다.** 도어가 닫혀 있는지와 발생한 열이 배출되고 있는지는 Function Controller가 직접 확인하며, 동작이 전달되는 사이에 해당 상태가 바뀔 수 있다.
...
- 조명 표시의 전환은 사용자가 지연으로 인지하지 않을 시간 안에 수행되어야 한다.
...
- Function Controller에는 이미 결정된 동작이 제공되어야 하며, 그 결정에 사용된 정보가 함께 제공될 필요는 없다.
- 동작 수준과 방향은 Function Controller가 해석 가능한 형태로 제공되어야 한다.
- Function Controller의 세부 동작 수행 방법은 상위 차량 기능이 직접 제어하지 않아야 한다.
- Function Controller가 제공하는 상태, 동작 결과 및 오류는 상위 차량 시스템에서 활용 가능해야 한다.
- Function Controller의 로컬 안전 판단에 의한 명령 거부는 정상 응답으로 처리되어야 한다.

---

## 10. 범위 요약

> **Function Controller는 확정된 실내 기능의 동작을 받아
> 도어 잠금, 실내 공기 순환 및 온도, 실내 조명을 실제로 동작시키고,
> 그 결과와 오류를 되돌려 주는 실행 시스템이다.**

Function Controller는 **동작 여부를 스스로 결정하지 않는다.** 측정값은 Function Controller에 제공되지 않으며, 판단은 상위 차량 시스템에서 수행된다.

다만 **자신이 직접 확인하는 정보에 근거한 안전 판단**은 Function Controller가 수행하며, 이 판단은 전달받은 동작을 수행하지 않을 수 있다.
