...
새로운 유효 도어 요청은 추가 구동이 없어도 목표 상태가 정상 확인되면 확인 이벤트를 생성한다. 같은 요청의 재전달·결과 조회·반복 상태 보고에는 새 확인 이벤트를 생성하지 않는다. 도어 이벤트의 발생·중복 구분은 [§2.2](#common-result-meaning), 지속 경고의 발생·해제는 [§3.2.4](#central-warning-rules)를 따른다. 탑승자 존재 관측과 잔류 탑승자 위험은 같은 정보가 아니다.
