...
버전: v0.24 · 상태: 공유용 초안 · 수정일: 2026-09-17
...
| 단발성 이벤트·음향(One-shot) | 도어 잠금 완료처럼 한 번 일어난 사실과, 그 사실을 알리기 위해 한 번 실행하는 음향 재생 |
...
| <a id="com-sys-evt-001"></a>COM-SYS-EVT-001 | 중앙의 도어 판단 기능은 유효한 요청과 BCM의 실행 확인 결과를 연결해야 한다. 도어의 잠금 상태가 실제로 목표 상태로 바뀐 경우에만 DOOR_LOCK_COMPLETE 또는 DOOR_UNLOCK_COMPLETE 이벤트를 생성해야 한다. 이미 정상적으로 목표 상태여서 장치를 움직이지 않고 완료한 DONE, 같은 요청의 재전달, REJECTED 또는 UNKNOWN만으로 새 완료 이벤트를 생성해서는 안 된다. 수용한 LOCK 요청을 실제로 실행한 뒤 목표 확인에 실패한 경우(NO_FEEDBACK 또는 DRIVE_LIMIT_EXCEEDED)는 DOOR_LOCK_ERROR로 알려야 하며, 잠금 해제 실패를 이 이름으로 알려서는 안 된다. 각 발생은 원 요청·대상 도어·종류와 연결된 식별을 유지해 중계·재전송 때 중복 재생되지 않아야 한다. VSS 재생에는 기존의 입력 유효성, 발생 후 경과 시간, 재생 대상 선택 규칙을 적용해야 한다. |
...
| 도어 사례 | 결과 | 새 도어 발생 |
|---|---|---|
| 유효한 새 요청·안전 조건 충족·NORMAL·이미 목표 상태 | DONE + ALREADY_AT_TARGET, 추가 구동 없음 | 없음 |
...
추가 구동 없이 요청을 완료하려면 새 요청을 판정하는 시점에 현재 상태가 유효하고 이미 목표에 도달했는지 확인해야 한다. 과거 중복 요청에 반환된 DONE은 과거 요청 결과이며 현재 도어 상태의 최신 증거가 아니다. 물리 완료의 정의와 VSS 실제 음향의 중재·전달 시간 충족은 별도다.
...
| 도어 발생 | 중앙이 BCM 실행 결과를 바탕으로 완료·실패를 판단하고 VSS가 이를 음향에 사용. **[잠정]** 사건 발생부터 음향 시작까지는 §7.6.3의 2초 미만 | 발생 식별·경과 시간 전달 방식·중재/실제 인지 기한 검증 필요 |
...
|---|---|---|---|---|
| <a id="central-c01"></a>도어 | MOBILE 요청, BCM 잠금·개폐·품질 | 중앙 등록 확인·목표/결과 연결 | BCM 실행; MOBILE 표시; VSS 도어 발생 소비 | 도어 결과·잠금/개폐는 구분 표시. 추가 구동 없이 완료 시 새 완료음 없음. 결과 대기는 §2.3.1의 잠정 3초. 물리 전달·기한 충족 미검증. |
...
|---|---|---|
| 도어 잠금·해제 | BCM의 잠금·개폐·품질·실행 노드의 동작 제한 조건. LOCK은 열린 도어 금지 | BCM 목표 확인 또는 정상 상태에서 이미 목표에 도달해 추가 구동 없이 완료한 경우. 실제 열림 상태와 새 완료음은 별도 |
...
| <a id="com-sys-dk-004"></a>COM-SYS-DK-004 | 중앙은 자동 잠금 해제의 발생 원인을 해당 도어 명령·BCM 결과에 연결하고, 실제 잠금 해제 전이가 확인된 경우에만 근접에 의한 해제 완료를 MOBILE에 제공해야 한다. 추가 구동 없이 완료한 DONE과 거부·실패·결과 확인 불가를 실제 해제로 표시해서는 안 된다. 도어 완료 음향은 COM-SYS-EVT-001의 기존 발생·중복 방지 규칙을 따라야 한다. |
...
| 4 | 안전 조건 충족·NORMAL·현재가 목표와 같음 | 추가 구동 없이 DONE + ALREADY_AT_TARGET |
...
| Door idempotency | 정상·허용된 새 동일 목표 요청은 추가 구동 없이 DONE + ALREADY_AT_TARGET 및 새 완료음 없음; 동일 식별 재전달은 기존 결과만 반환 |
...
- 실내 온도·습도·조도 측정
- 후방 물체와의 거리 측정 및 근접 위험 상태(`CAUTION`/`EMERGENCY`/`CLEAR`) 판단
- 판정·측정 결과의 유효성 확인
- 중앙 제어 및 VSS로의 정보 제공
- 자체 오류 검출 및 상태 제공
...
| <a id="cis-sys-fun-023"></a>CIS-SYS-FUN-023 | CIS는 제공하는 각 값과 후방 위험 상태에 유효 여부, 원본 시점/경과 시간, 갱신 구분 및 확인 불가 사유를 연결하여 제공해야 한다. 정상 값이 없는 경우에도 사용 가능한 경로로 품질·진단 정보를 제공해야 하며, 논리 의미와 인코딩은 구분해야 한다. |
...
| <a id="cis-sys-fun-026"></a>CIS-SYS-FUN-026 | CIS는 후방 위험을 판단하고 현재 상태를 `CAUTION`/`EMERGENCY`/`CLEAR` 중 하나로 제공해야 한다. 이 상태 이름은 VSS에서 사용하는 이름과 같아야 한다. 상태 변경이나 재연결 후 상태를 다시 맞출 때는 §5.5.1의 품질·경과 시간·순서 근거를 보존해야 하며, 같은 정보의 반복 전달을 새 위험 발생으로 처리해서는 안 된다. 후방 거리와 임계값은 내부 판단에만 사용하고 중앙·VSS 운영 인터페이스에 직접 제공하지 않아야 한다. |
...
다음은 논리 의미이며 특정 enum·비트 배치를 강제하지 않는다. 값의 `VALID`는 활성 조건, 새 입력의 품질·유효 범위, 최신성 및 필요한 복구 확인이 모두 충족된 경우에만 사용한다. 최초 미수신은 `NO_DATA`, 현재 입력의 검증 실패는 `INVALID`, 이전 유효 입력의 허용 경과 시간 만료는 `STALE`로 구분한다. 비활성 및 활성 조건 확인 불가는 별도 서비스 사유로 제공한다. 어느 경우도 숫자 0, 탑승자 없음 또는 `CLEAR`로 대체하지 않는다.
...
위험 값의 변화뿐 아니라 품질·활성 변경도 정보 사용 기능이 알 수 있게 제공한다. 상태의 주기·최신성은 §2.5의 잠정값을 적용하고, 전달·경과 시간 표현은 인터페이스 설계에서 정한다. 상태가 같다는 이유로 정보 사용 기능이 최신성을 확인할 수 없게 해서는 안 된다. 완전 단절은 정보 사용 기능의 감시로 검출한다. VSS에서 입력을 신뢰할 수 없을 때 재생 유지/종료·대기는 §7.6을 따르며, 음향 종료를 CIS의 정상 위험 해제로 역변환하지 않는다.
...
> **[검토 필요]** 후방에 물체가 없는 상태와 센서가 측정하지 못한 상태를 구분할 수 있는지 시험해야 한다. 경고를 나눌 거리 기준, 센서 전원 상태를 확인하는 입력, 측정값을 사용할 수 있는 시간도 확인해야 한다. 요청 후 결과를 기다리는 시간은 SysRS §2.3.1의 임시값을 따른다. 실제 동작과 결과 전달에 걸리는 시간, 경고가 전달되어 사용자가 알아차리기까지의 시간은 시험해야 한다.
...
| VSS의 신뢰할 수 없는 입력·비활성 수용과 재생 정책 | CIS의 정보 제공과 VSS의 정보 사용 연결, §7.6.2 잠정 Hold | 후방 300ms, 끼임/잔류 1초 유지 뒤 중단·미확인. 경과 시간·재생·통합 성능 미검증 |
...
| 담당 | 확인할 내용 | 기준 위치 |
|---|---|---|
| CIS·후방 센싱 | 물체가 없는 상태와 측정 실패의 구분, 경고 거리, 전원 입력, 측정값의 사용 가능 시간 | [CIS 확인 항목](#cis-open-parameters) |
...
| <a id="vss-sys-fun-030"></a>VSS-SYS-FUN-030 | 유효한 실제 잠금 완료 Event에 잠금 완료 단발성 음향을 생성하고 FUN-011을 적용해야 한다. 이미 목표 상태여서 추가 구동 없이 완료한 경우에는 새 음향을 생성하지 않아야 한다. |
| <a id="vss-sys-fun-031"></a>VSS-SYS-FUN-031 | 유효한 실제 잠금 해제 완료 Event에 잠금 완료와 구분 가능한 단발성 음향을 생성하고 FUN-011을 적용해야 한다. 이미 목표 상태여서 추가 구동 없이 완료한 경우에는 새 음향을 생성하지 않아야 한다. |
...
| <a id="vss-sys-int-004"></a>VSS-SYS-INT-004 | VSS ECU에 제공되는 후방 장애물 정보는 원시 거리값(Raw Data)이 아니라, 외부에서 위험 수준을 판단한 결과를 기본으로 해야 한다. |
...
- 잠금 완료/실패 및 사용 시작/종료 Event의 발생 경과 시간, 중복 이력과 Wake 전원 유지
...
후방 장애물 기능에서는 VSS가 초음파 거리값을 직접 판정하지 않고 외부에서 확정된 **CLEAR / CAUTION / EMERGENCY 위험 상태**를 음향으로 표현한다.
...
| 도어 완료/이상 발생 | 중앙 도어 의미 판단 기능 | VSS | 단발성 이벤트의 발생 식별·경과 시간 | §2.2 실제 전이/실행 실패 기준 적용; 추가 구동 없음·거부·UNKNOWN은 새 발생 아님 |
...
| 근접 자동 해제 | **[잠정]** ESP32 등록·근접 → 중앙 설정·허용 판단 | BCM 실행·MOBILE 표시 | 사용 설정·현재 사용 가능 여부·확정 해제 원인 | §3.2.6. 기존 VSS 도어 완료음 연결; 별도 음향·조명 알림 종류 추가 없음 |
...
- VSS는 `주의 / 긴급 / 해제` 현재 상태와 기능 활성 여부·품질·경과 시간 근거를 함께 사용한다. [§5.5.1](#sys-cis-s05)의 동일 판정·순서·복구 계약을 적용하며 신뢰할 수 없는 상태를 CLEAR로 변환하지 않는다.
...
- CIS는 후방 근접 위험 상태를 VSS가 정의한 이벤트 이름과 동일하게 유지한다.
...
도어를 추가로 구동하지 않고 요청을 완료한 경우에는 새 이벤트를 생성하지 않는다. 도어 이벤트의 발생·중복 구분은 [§2.2](#common-result-meaning), 지속 경고의 발생·해제는 [§3.2.4](#central-warning-rules)를 따른다. 탑승자 존재 관측과 잔류 탑승자 위험은 같은 정보가 아니다.
...
후방 거리 측정·필터링·주의/긴급 임계값 판단은 CIS가 담당한다. VSS는 원시 거리 `distance_cm`을 입력으로 요구하지 않으며, 확정된 의미·품질·가용 상태를 사용한다. `CAUTION → EMERGENCY`, `EMERGENCY → CAUTION`, 정상 `CLEAR`는 현재 상태의 변경으로 처리하고 재생 대상을 다시 선택한다. 해당 기능이 비활성화됐다는 유효한 정보와 입력을 신뢰할 수 없는 상태는, 위험이 없음을 확인한 정상 해제와 구분한다.
