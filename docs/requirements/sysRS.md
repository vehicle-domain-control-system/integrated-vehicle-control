...
| <a id="com-sys-ctr-002"></a>COM-SYS-CTR-002 | **[잠정]** 중앙은 §3.2.3~3.2.4의 입력 계약과 발생·해제 조건에 따라 차량 사용 의미와 잔류 탑승자·끼임·이탈 후 열린 도어 경고를 판단해야 한다. 끼임 감지·즉시 보호·실제 반전 결과·현장 확인 전달은 WINDOW가, 탑승자 관측과 품질은 CIS가 제공해야 한다. 후방 근접 위험의 의미 판단은 중앙이 담당하며, CIS는 판단에 필요한 후방 거리 원시값과 유효성만 제공해야 한다. 각 경고의 상태·품질·모의 여부를 구분하고, 앱 확인·미수신·음향 종료만으로 위험 해제를 생성해서는 안 된다. 실제 입력이 미구성인 기능의 모의 시연을 실제 감지 지원으로 표시해서는 안 된다. |
...
|---|---:|---:|
| 후방 거리·활성·품질: CIS → 중앙, 중앙 → VSS의 후방 위험 상태, 중앙 → CIS의 후방 활성 허용 | 200 ms | 500 ms |
...
| 영역 | 담당 책임 | 관련 영역의 책임 |
|---|---|---|
| 중앙 | 단말 등록 확인·확정 설정, 자동 기능·자원 중재, 차량 사용 의미·잔류 위험·끼임 경고·이탈 후 도어 경고의 판단, 후방 근접 위험의 의미 판단, 결과 연결·사용자 표시용 상태 매핑 | 직접 센서 측정·로컬 끼임/전기 보호·음향 내부 재생 중재 |
| BCM | 도어·공조·실내 조명의 명령 실행, 직접 관측 가능한 적용 상태·결과·자체 보호 | 사용자 설정의 최종 결정·관리·자동 공조 정책·센싱 정보 중계 |
| CIS | 실내 관측·품질, 후방 거리의 내부 측정과 유효성 확인 | 후방 근접 위험의 의미 판단·잔류 위험의 차량 수준 판단·음향 출력 |
...
| <a id="central-c06"></a>후방 경고 | CIS 거리(raw)·품질 | 중앙 위험 의미 판정(주의/긴급/해제) | VSS 후방 경고 | 중앙이 CIS의 거리·유효성을 근거로 판정. CIS는 원시 거리·임계값을 운영 외부(VSS 등)에 직접 제공하지 않음. 경계/신뢰할 수 없는 상태는 §5.6, 음향 출력은 §7.6을 따름 |
...
| 후방 위험 | CIS 거리(raw)·품질, 중앙 의미 판정, VSS 재생과 신뢰할 수 없는 입력 처리 | 센서 능력, 전원/선택 기어 입력, 거리·최신성·Hold 실측, 중앙 임계값 확정 | CIS+중앙+VSS·인터페이스 |
...
- 후방 물체 거리를 위험 수준(`CAUTION`/`EMERGENCY`/`CLEAR`)으로 분류하는 판단 — 중앙 제어의 책임
...
후방 근접 기능의 경우 CIS는 거리 측정과 필터링까지만 수행하고, 위험 수준 판단(`CAUTION`/`EMERGENCY`/`CLEAR`)은
중앙 제어가 CIS가 제공한 거리값과 유효성을 이용해 수행한다.
...
| <a id="cis-sys-fun-016"></a>CIS-SYS-FUN-016 | CIS는 특정 센서 또는 비전 기능에 오류가 발생하더라도, 오류와 무관한 다른 판정·측정 기능을 불필요하게 중단하지 않아야 한다. |
| <a id="cis-sys-fun-017"></a>CIS-SYS-FUN-017 | CIS는 통신 오류 동안 마지막 정상 값을 현재 정상 값으로 표시하지 않아야 한다. |
| <a id="cis-sys-fun-018"></a>CIS-SYS-FUN-018 | CIS는 유효한 탑승자 판정 결과, 환경 측정값 및 후방 거리 측정값을 정의된 주기로 중앙 제어에 전송해야 한다. |
| <a id="cis-sys-fun-019"></a>CIS-SYS-FUN-019 | CIS는 제공하는 각 값(후방 거리 포함)에 유효 여부, 원본 시점/경과 시간, 갱신 구분 및 확인 불가 사유를 연결하여 제공해야 한다. 정상 값이 없는 경우에도 사용 가능한 경로로 품질·진단 정보를 제공해야 하며, 논리 의미와 인코딩은 구분해야 한다. |
| <a id="cis-sys-fun-020"></a>CIS-SYS-FUN-020 | CIS는 사용 가능한 경로로 통신 오류와 영향 받는 제공 경로를 식별 가능하게 제공해야 한다. 완전 단절된 경로로 오류 통지를 보장하지 않아야 하며, 그 경로의 정보 사용 기능은 §5.7.1의 미수신 감시로 단절을 확인해야 한다. |
| <a id="cis-sys-fun-021"></a>CIS-SYS-FUN-021 | CIS는 해당 제공 경로의 통신 복구 후 획득한 새 유효 값과 해당 기능의 복구 조건이 모두 확인된 경우에만 그 값의 정상 제공을 재개해야 한다. 복구된 경로의 품질·진단 제공은 센서 정상화를 기다리지 않아야 하고, 정상 기능의 재개를 다른 기능의 고장 때문에 지연하지 않아야 한다. |
| <a id="cis-sys-fun-022"></a>CIS-SYS-FUN-022 | CIS는 실내 영상을 탑승자 인식 목적 범위를 벗어나 저장하거나 외부로 전송하지 않아야 한다. |
...
`REAR_DISTANCE`와 그 유효성은 중앙 제어가 후방 근접 위험(`CAUTION`/`EMERGENCY`/`CLEAR`)을 판단하는 데 사용하는
입력이다. CIS는 이 값을 이용해 스스로 위험 수준을 분류하지 않는다. VSS로의 직접 전달은 없으며, 판단된 위험 상태를
VSS에 전달하는 것은 중앙 제어의 역할이다.

> 실제 신호명, 숫자 값, 메시지 배치 및 전송 방식은 후속 인터페이스 설계에서 확정한다.

#### 5.5.2 후방 근접 정보의 CIS 내부 처리 개념

아래 도식은 활성·유효 판정이 성립한 경로를 보인 것이다. 품질·활성 확인 불가 경로는 §5.4.1을 따른다.

CIS는 거리를 측정하고 필터링하여 raw 값과 유효성만 중앙 제어에 제공한다. 거리를 위험 수준으로 분류하는 판단은
중앙 제어가 수행하며, 그 결과를 VSS 등 다른 기능에 전달하는 것도 중앙 제어의 역할이다.

```mermaid
flowchart TD
    US["초음파 센서 · Raw Distance"]
    FIL["필터링 / 노이즈 처리"]
    CENTRAL["중앙 제어 · 위험도 판단"]

    US --> FIL -->|"거리(raw) + 유효성"| CENTRAL
```

> 이 다이어그램은 정보의 **의미 흐름**을 표현한다.  
...
### 5.6 후방 거리 측정 범위 및 품질 후보

CIS가 중앙 제어에 제공하는 후방 거리(raw)의 측정 범위와 품질 판정만 정의한다. 거리를 위험 수준
(`CAUTION`/`EMERGENCY`/`CLEAR`)으로 분류하는 임계값과 판단 로직은 중앙 제어의 책임이며 본 절의 범위 밖이다.

| 입력 조건 | 거리(raw) 품질 |
|---|---|
| 유효 범위 `10 ≤ d ≤ 100 cm` 내 새 측정, 센서 품질·경과 시간 기준 충족 | VALID |
| 검증된 정상 미감지 (지원 센서에서 정상 진단과 함께 확인) | VALID (정상 미감지로 구분 제공) |
| `d < 10 cm`, `d > 100 cm`, 비정상 수치·센서 진단 실패 | INVALID |
| 측정 회차 무응답·초기 값 없음·원본 정보 경과 시간 만료 | 해당 NO_DATA/INVALID/STALE |
| 전원 불허·선택 후진 구성의 비후진 | 후방 감지 비활성 |
| 필수 활성 입력 확인 불가 | 후방 감지 활성 확인 불가 |

현재 후보의 측정 지원 범위는 `10~100cm`다. `101cm`처럼 범위 밖 숫자를 정상 미감지로 취급하지 않는다. 실제 센서가 검증된 무물체 상태를 구분할 수 없다면 정상 미감지 구분을 제공할 수 없고 해당 결과는 확인 불가다. 특히 10cm 미만의 근접을 이 센서 후보만으로 검출한다고 보장하지 않는다. 지원 범위 확대는 센서 선정·검증 뒤 후보 표와 요구를 함께 갱신해야 한다.

| 항목 | Candidate / 현재 선택 |
|---|---|
| 내부 측정 거리 단위·범위 | `cm`, `[CANDIDATE] 10 ≤ d ≤ 100` |
| 거리 필터링 | 방식·노이즈 기준 미정 (CIS 담당) |

거리를 위험 수준으로 분류하는 임계값(긴급/주의 경계)과 히스테리시스 정책은 중앙 제어 쪽 상세 요구사항에서
정의한다. 현재는 참고용으로 기존 후보 10/40/70/100cm 구간 분할이 논의되었다는 사실만 남긴다 — 최종 확정은
중앙 제어의 책임이다.
...
| <a id="cis-sys-per-003"></a>CIS-SYS-PER-003 | 후방 거리(raw) 값이 CIS 내부에서 유효로 확정된 시점부터 그 값과 품질을 CIS 논리 제공 경계에 준비하는 시점까지의 지연을 제한해야 한다. 센싱 이전 지연·네트워크 전송·중앙 제어의 위험 판단·VSS 중재 및 실제 음향 시작은 이 시간에 포함하지 않아야 한다. | `[CANDIDATE] ≤ 100 ms` |
...
| PER-003 | 후방 거리(raw) 유효 확정 → 논리 제공 준비 | 후보 100ms; 200ms 주기 대기 때문에 초과하지 않음 |
...
| ID | 요구사항 |
|---|---|
| <a id="cis-sys-int-001"></a>CIS-SYS-INT-001 | CIS는 중앙을 통해 후방 감지 활성 판단에 필요한 차량 전원 허용 상태와 그 품질·최신성을 제공받아야 한다. 기본 구성은 유효한 전원 허용 동안 후방 감지를 활성화하며, 불허와 허용 확인 불가를 구분하고 어느 경우에도 정상 유효 거리 측정으로 치환하지 않아야 한다. |
...
비활성·확인 불가 전환 시 유효 거리 제공을 종료하고 사유를 제공한다. 이를 정상 거리 값 제공으로 대체하지 않는다. 재활성 후에는 그 이후의 새 입력으로 다시 측정한다. 이전 값을 자동 복원하지 않는다. 후진 연계 설정은 구성 값이며 모바일의 신규 사용자 제어 항목으로 추가하지 않는다. 이 후방 활성 조건을 탑승자·환경 센싱에 일괄 적용하지 않는다.
...
| <a id="cis-sys-int-005"></a>CIS-SYS-INT-005 | CIS는 후방 물체 거리(raw)와 그 유효·품질·최신성 및 갱신 구분을, 중앙 제어가 근접 위험 수준을 판단하는 데 사용할 수 있도록 §5.5.1에 따라 제공해야 한다. CIS는 위험 수준을 스스로 분류하지 않으며, 음향을 직접 출력하거나 재생 우선순위를 결정하지 않아야 한다. |
...
| <a id="cis-sys-saf-002"></a>CIS-SYS-SAF-002 | CIS는 신뢰할 수 없는 후방 거리 측정값을 유효한 값으로 확정해서는 안 된다. |
| <a id="cis-sys-saf-003"></a>CIS-SYS-SAF-003 | CIS는 정상적인 탑승자/환경/후방 거리 정보를 제공할 수 없는 경우 해당 상태가 외부 시스템에서 식별 가능해야 한다. |
| <a id="cis-sys-saf-004"></a>CIS-SYS-SAF-004 | CIS는 실내 영상 원본과 센서 원시 데이터를 어떠한 외부 인터페이스로도 노출하지 않아야 한다. 검증·가공된 탑승자 결과와 온도·습도·조도, 후방 거리 및 유효성은 정해진 논리 출력 범위로 제공해야 한다. |
...
| 후방 측정 실패 | 활성 조건 충족 및 센서 진단 회복 | 새 유효 거리 또는 검증된 정상 미감지의 확인 | 유효 거리 미제공, 과거 값은 이력으로만 유지 |
...
| ID | 요구사항 |
|---|---|
| <a id="cis-sys-nfr-005"></a>CIS-SYS-NFR-005 | CIS는 논리 인터페이스와 통신 구현을 분리하여 실제 프로토콜 변경이 판정 로직을 불필요하게 바꾸지 않도록 해야 한다. 상위 차량 기능은 정의된 활성 조건을 제공할 수 있으나 CIS 내부 인식 모델·센싱 전처리·필터 처리 방법을 직접 제어하지 않아야 한다. |
...
| Measurement range | 10~100cm 범위 내 유효 거리는 그대로 raw 값과 유효성으로 중앙에 제공; 분류·임계값 판단 없음 |
| Outside range | 9/101cm는 확인 불가로 표시, 임의의 유효 거리로 치환하지 않음 |
| No object | 정상 무물체 지원·진단이 검증된 회차만 정상 미감지로 제공. echo 없음·센서 분리는 확인 불가 |
...
| Path isolation | 특정 값(예: 후방 거리)의 제공 경로만 단절이면 그 경로 오류 표시, 나머지 정상 제공 유지 |
...
| Reactivation | 비활성/신뢰할 수 없음 전환은 유효 거리 제공 없음; 재활성 이후 새 입력으로 재측정 |
| Resynchronization | 값 손실 뒤 최신 유효 거리·품질 재제공, 과거 값을 순서대로 반복 재생하지 않음 |
...
| 시간 | Rear distance internal-to-external delay | ≤ 100 ms |
...
|---|---|---|
| 센서 모델·유효 범위·정상 무물체 구분 | CIS/HW: 사양과 단선/무물체/근거리 대조 실측 | 10cm 미만 및 범위 밖 감지 보장 없음. 정상 미감지 능력 없으면 해당 구분 제공 불가 |
...
| 거리 필터 방식·복구 2회 | CIS: 경계·노이즈·간헐 오류 대조 | 신규 복구 횟수는 후보; 추가 히스테리시스는 현재 미적용. 위험 임계값 자체는 중앙 담당 |
...
| 후방 위험 임계값 확정 및 VSS 재생 정책 | 중앙: 거리(raw)·유효성 기반 임계값 판단, §7.6.2 잠정 Hold와 연계 | 후방 300ms, 끼임/잔류 1초 유지 뒤 중단·미확인. 경과 시간·재생·통합 성능 미검증 |
...
> 유효성이 확인된 정보만 중앙 제어로 제공하며,
> 자신의 상태와 오류를 관리하는 실내 센싱 모듈이다.**

후방 근접 기능에서는 CIS가 거리 측정과 유효성 확인까지만 수행하며, 그 값을 위험 수준
(주의/긴급/해제)으로 분류하는 판단은 중앙 제어가 수행한다.
...
- 후방 위험 판단 담당은 중앙 제어이다(CIS는 거리·유효성만 제공). VSS 소비 규칙은 §7.6을 따른다. 센서 검증·후보 수치·물리 전달 경로는 미정
...
| 후방 거리(raw) | CIS | 중앙 | 거리값(cm)+유효성 | CIS는 위험 수준을 분류하지 않음 |
| 후방 위험 현재 상태 | 중앙 (CIS 거리·유효성 근거) | VSS | CLEAR/CAUTION/EMERGENCY+품질 | 감지 불가≠CLEAR, 현재 상태 재동기화 |
...
> CIS의 인터페이스 대상은 중앙 제어 하나뿐이다. 후방 위험 상태를 VSS가 필요로 하는 경우,
> 그 전달은 중앙 제어가 담당하며 본 절의 범위 밖이다.
...
| 후방 물체 거리 (raw) | 중앙 제어의 근접 위험 수준 판단에 사용 | Central Controller | 미정(TBD) |
| 각 정보의 활성·유효 여부·원본 정보 경과 시간·갱신 구분 | 정상·신뢰할 수 없음·비활성 및 재동기화 구분 | Central Controller | 미정(TBD) |
| 기능/경로별 현재 오류·복구·이력 | 부분 사용 가능 여부 구분 | Central Controller | 미정(TBD) |
...
flowchart TD
    US["Ultrasonic Sensor"]
    CIS["CIS · 거리 측정 · 필터링"]
    CENTRAL["Central Controller · 위험도 판단"]
...
- 거리 임계값 및 위험도 판정은 중앙 제어에서 정의한다.
- 판단된 위험 상태(`주의`/`긴급`/`해제`)를 VSS에 전달하는 것은 중앙 제어의 역할이며 본 절의 범위 밖이다.
...
- 센서 Raw Data라도 그 값을 실제로 필요로 하는 소비자(중앙 제어)가 있는 경우에는 유효성과 함께 전달한다.
...
> 후방 근접 위험 임계값(주의/긴급 거리 등)은 중앙 제어가 정의할 항목이며, 본 CIS 절의 범위 밖이다.
...
| 중앙 제어 → VSS | 후방 위험의 현재 의미·품질·가용 상태 | 중앙이 CIS의 후방 거리(raw)·유효성을 근거로 의미 판단. 실제 전달 노드는 미정 |
| VSS → 외부 | 동작 상태·입력 수용 가능 여부·서비스 가용성·고장 | VSS 생산. 가용성·현재 고장·고장 이력은 중앙을 통해 MOBILE에 제공 |

의미 판단 담당과 물리 전달 노드는 구분한다. CIS는 후방 거리와 유효성만 제공하며, 위험 의미 판단은 중앙이 한 번만 수행한다. 차량 사용·중앙 경고의 조건은 [§3.2.3](#central-input-profile)·[§3.2.4](#central-warning-rules)을 따른다.
...
| `REAR_OBSTACLE_STATE` | 지속 상태: `CLEAR / CAUTION / EMERGENCY` | 주의 / 긴급 | 중앙, CIS 거리·유효성 근거 |
...
| 기능 가용 상태 | 후방의 유효 DISABLED·UNCONFIRMED와 정상 CLEAR 구분 | [중앙 판단 근거](#sys-cis-s05) 적용, 실제 표현 `TBD` |
...
후방 거리 측정·필터링은 CIS가, 주의/긴급 임계값 판단은 중앙 제어가 담당한다. VSS는 원시 거리 `distance_cm`을 입력으로 요구하지 않으며, 확정된 의미·품질·가용 상태를 사용한다. `CAUTION → EMERGENCY`, `EMERGENCY → CAUTION`, 정상 `CLEAR`는 현재 상태의 변경으로 처리하고 재생 대상을 다시 선택한다. 해당 기능이 비활성화됐다는 유효한 정보와 입력을 신뢰할 수 없는 상태는, 위험이 없음을 확인한 정상 해제와 구분한다.
...
후방 거리 필터·센서 성능은 CIS의 [§5.6](#sys-cis-s06)·[§5.14](#sys-cis-s14)에서, 위험 임계값은 중앙 제어에서 관리한다. VSS 인터페이스에서 원시 거리나 임계값을 별도로 결정하지 않는다.
