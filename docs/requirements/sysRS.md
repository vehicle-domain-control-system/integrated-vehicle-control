...
버전: v0.20 · 상태: 공유용 초안 · 작성일: 2026-09-16
...
아래는 기능별 대상과 요구 정의 수다. 중앙과 VSS의 보드 공유 여부, 외부 조명 배치와 실제 통신 경로는 미정이다.
...
| 유효성 / 최신성 / 인증 | 의미 사용 가능, 원본 정보 경과 시간과 수신 후 경과 시간, 출처·권한을 구분 |
| 후보 / 설계 선택 | 후보 숫자는 검증 전 값. 현재 설계 선택도 부품·성능 검증 완료를 뜻하지 않음 |
| DEFERRED | 현재 구현 의무에 포함하지 않는 후속 후보 |

BCM은 무관한 새 요청 오류와 현재 유효 동작을 구분하며 권한·유효성 상실 및 로컬 보호를 따로 판단한다. WINDOW는 상위 이동 정지, 현재 저전압 ELS 데모는 OFF, VSS는 제한 Hold 후 중단을 적용한다. 이 동작을 하나의 공통 OFF 또는 무조건 유지 규칙으로 바꾸지 않는다.

### 주요 용어

| 용어 | 이 문서의 의미 |
|---|---|
| 중앙 제어 | 차량 수준의 권한·허용·중재·정책을 담당하는 논리 기능. 상위·Domain·중앙처리장치는 이 기능의 별칭 |
...
요청·권한·품질의 공통 동작은 [SR 공통 요구](SR.md#sr-common-request)를 구체화한다. 실제 ECU 배치·암호 방식·메시지 비트 폭은 별도 설계에서 정한다.
...
| <a id="com-sys-aut-001"></a>COM-SYS-AUT-001 | 중앙의 차량 측 권한 판단 기능은 요청 수용 시 인증된 사용자, 대상 차량, 세션 유효성 및 해당 기능의 제어 권한을 확인해야 한다. 차량 A의 권한을 차량 B의 허용 근거로 사용해서는 안 된다. MOBILE은 차량 측 판정과 그 유효 범위를 반영해야 하며 자체 인증 성공이나 기능 허용을 만들어서는 안 된다. 중계의 전달 성공은 권한 확인을 대체하지 않아야 한다. |
...
| ACCEPTED | 중앙이 인증·권한·정의된 사전 조건에 따라 처리를 수용함. 실행 노드의 최종 수용/구동을 보장하지 않음 |
| IN_PROGRESS | 실행 노드의 실제 진행 또는 해당 설정 처리의 진행이 확인됨 |
| DONE | 해당 요청의 목표가 확인됨. 설정 요청은 반영, 시작/정지는 해당 동작 상태 확인이며 공조 목표 온도 도달까지 의미하지 않음 |
| REJECTED | 실행 시작 전에 권한·유효성·인터록 등으로 거부됨. 중앙 수용 뒤 로컬 실행 전 거부도 포함 |
...
기존 요청의 중복 상태/결과 반환 및 조회도 인증된 대상 차량·사용자 권한 범위 안에서만 제공한다. 만료·무효 세션은 새 동작을 수행하지 않으며, 재인증 후 과거 요청 조회에 필요한 접근 권한을 다시 확인한다.
...
| 인증/권한 실패, 무효 명령, 신뢰할 수 없는 상태, LOCK 시 문 열림 | REJECTED + 사유 | 없음 |
...
|---|---|---|
| 최초 처리 응답 기한 | **[잠정]** SENT부터 3 s. 전달 ACK 제외, 유효한 ACCEPTED/IN_PROGRESS 또는 최종 결과로 종료 | 중앙·MOBILE 담당: 무선 왕복+권한/접수 처리 실측으로 조정 |
...
| 결과 순서·출처 | 동일 요청·권한 있는 출처·인과 순서가 확인되는 결과만 반영 | 인터페이스 담당: 재부팅 세대·상태 순서 식별·결과 조회 응답 최신성 형식 확정 |
...
| <a id="com-sys-ctr-002"></a>COM-SYS-CTR-002 | **[잠정]** 중앙은 §3.2.3~3.2.4의 입력 계약과 발생·해제 조건에 따라 차량 사용 의미와 잔류 탑승자·끼임·이탈 후 열린 도어·엔진룸 동물 경고를 판단해야 한다. 끼임 감지·즉시 보호·실제 반전 결과·현장 확인 전달은 WINDOW가, 탑승자 관측과 품질은 CIS가 제공해야 한다. 엔진룸 감지 입력은 CIS와 분리해야 한다. 후방 근접 위험의 의미 판단은 CIS가 담당하며 중앙은 이를 중복 판정해서는 안 된다. 각 경고의 상태·품질·모의 여부를 구분하고, 앱 확인·미수신·음향 종료만으로 위험 해제를 생성해서는 안 된다. 실제 입력이 미구성인 기능의 모의 시연을 실제 감지 지원으로 표시해서는 안 된다. |
| <a id="com-sys-ctr-003"></a>COM-SYS-CTR-003 | 중앙의 명령 수용·중재는 실행 노드의 로컬 보호를 해제하거나 지연시키는 근거가 되어서는 안 된다. BCM·WINDOW·EXTERIOR_LIGHT는 해당 기능의 직접 관측과 보호 조건에 따라 출력을 제한하고 그 상태·결과를 제공해야 한다. 통신 상실·권한 철회·복구 시 출력은 기능별 계약을 따라야 하며 모든 기능에 동일한 OFF 또는 자동 재개를 강제해서는 안 된다. VSS는 확정된 의미의 재생을 중재하고 센서 위험 판단이나 액추에이터 보호를 대신하지 않아야 한다. |
...
| CIS 온도·습도·실내 조도, 별도 외부 조도·엔진룸 감지 → 중앙; 중앙의 엔진룸 경고 → 정보 사용 기능 | 500 ms | 2,000 ms |
...
유지 허용은 새 실행 명령과 별도의 논리 상태다. 대상 기능·현재 목표/작업 식별·새 평가 순서·나이·허용 여부를 구분한다. 중앙은 현재 차량 권한·필수 입력·실구동 허용을 확인할 수 있을 때만 갱신한다. 같은 평가의 재전달, 무관한 상태 수신 또는 다른 목표의 허용으로 현재 목표를 연장하지 않는다. 갱신은 구동 재시작·재시도 횟수·작업 타이머·결과 대기를 초기화하지 않는다. 입력 근거 자체의 나이도 보존하며 모의 정보로 실제 구동 허용을 만들지 않는다.
...
이 감시는 중앙이 선택한 실행 예정·활성 상위 출력 목표에만 적용한다. 출력이 없는 정상 대기·저장된 설정·대기 중 예약에는 지속 실행 명령을 요구하지 않는다. 앱 연결 단절은 차량 권한 철회와 구분하며 중앙의 유지 허용을 앱의 주기 메시지로 대신하지 않는다. ELS 임시 점등의 10초와 선행/환기의 5분 상한은 허용 갱신으로 늘어나지 않는다.
...
| 영역 | 담당 책임 | 관련 영역의 책임 |
|---|---|---|
| 중앙 | 차량별 권한·확정 설정, 자동 기능·자원 중재, 차량 사용 의미·잔류 위험·끼임 경고·이탈 후 도어 경고의 판단, 결과 연결·사용자 표시용 상태 매핑 | 직접 센서 측정·로컬 끼임/전기 보호·음향 내부 재생 중재 |
| BCM | 도어·공조·실내 조명의 명령 실행, 직접 관측 가능한 적용 상태·결과·자체 보호 | 사용자 설정의 최종 소유·자동 공조 정책·센싱 정보 중계 |
| CIS | 실내 관측·품질, 후방 거리의 내부 측정과 위험 의미 판단 | 잔류 위험의 차량 수준 판단·엔진룸 감지·음향 출력 |
...
| MOBILE | 사용자 요청 생성, 차량에서 확인한 설정·상태·결과·경고·가용성 표시 | 자체 차량 권한 승인·위험 추정·차량 상태 보정·로컬 출력 제어 |
...
|---|---|---|---|---|
| <a id="central-c01"></a>도어 | MOBILE 요청, BCM 잠금·개폐·품질 | 중앙 권한·목표/결과 연결 | BCM 실행; MOBILE 표시; VSS 도어 발생 소비 | 도어 결과·잠금/개폐는 구분 표시. 무구동 완료 시 새 완료음 없음. 결과 대기는 §2.3.1의 잠정 3초. 물리 전달·기한 충족 미검증. |
...
| <a id="central-c04"></a>실내 조명 | MOBILE 설정, CIS 실내 조도, 중앙 알림 | 중앙 확정 설정·알림/사용자 목표 중재 | BCM 적용·보호; MOBILE 표시 | 사용자 설정·BCM 적용·물리 확인 미지원은 구분 표시. 실제 색상 표현·수치는 미정. |
...
| <a id="central-c13"></a>이탈 후 도어 경고 | BCM 개폐·품질, 차량 상태 입력의 사용자 이탈/복귀 | 중앙 이탈+열린 도어 판단 | MOBILE 권한 있는 사용자 | **[잠정]** §3.2.4의 발생·해제 적용. 앱 단절을 이탈로 사용하지 않음. 실제 이탈 감지는 미구성. |
| <a id="central-c14"></a>엔진룸 동물 경고 | 별도 엔진룸 감지 입력 기능; CIS 제외 유지 | 중앙 경고 상태 연결 | MOBILE | **[잠정]** 유효 존재/없음 관측으로 발생/해제. 시연은 모의 입력, 실제 센서·판정 성능은 미정. |
| <a id="central-c15"></a>인증·연결·결과 | MOBILE 요청; 차량 측 인증/권한 근거; 실행 노드 결과 | 중앙 차량 권한·확정 결과·표시용 변환 | MOBILE | 제한 재연결·재인증/조회·로컬 등록 해제를 제공. 백그라운드 알림은 선택. 권한 폐기 절차·기한·물리 인증 배치 미정. |
...
Fan과 온도 목표가 함께 전달되지 못한 경우에도 BCM은 확인된 Fan 허용 없이 온도 출력을 시작하지 않는다. 중앙은 필요한 Fan 정상 상태를 받은 뒤 새 온도 지시를 제공한다. 이미 진행 중인 동작의 요청 만료·권한 철회는 중단과 연결하고, 무관한 새 요청 오류만으로 정상 동작을 취소하지 않는다. 명령 수용·유지 허용은 §2.6의 잠정값을 적용하고 실제 전달 방식과 충족 여부는 확인한다.

실내 조명은 중앙이 현재 알림을 선택한다. NORMAL에는 사용자 사용 여부·지원 밝기·색상을 적용하고, 밝기를 별도로 지정하지 않은 자동 구성에는 실내 조도 기준을 사용한다. 사용자 OFF는 NORMAL만 끄며 기존 안전·상황 알림 우선순위는 유지한다. 상위 알림 종료 후에는 최신 NORMAL 설정으로 돌아가고, 중단된 일시 알림은 재개하지 않는다. 색상 표현·알림별 패턴과 표시 시간은 후속 설계에서 정한다.

<a id="central-mobile"></a>
### 3.2.2 모바일 요청과 차량 결과 연결

중앙은 모든 요청에 차량·사용자·기능별 권한과 현재 구성의 지원 여부를 확인한다. MOBILE은 이 판정을 표시하며 무관한 센서의 STALE 하나로 전체 제어를 막지 않는다. 미지원 입력·정책은 사용할 수 없다는 사유로 제공한다.
...
권한 철회는 중앙이 새 요청을 차단하고, 해당 권한에 의존한 유지 동작에는 기존 기능별 중단 규칙을 적용한다. 단순 앱 단절·화면 비활성화와 차량 권한 철회는 같은 사건이 아니다. 연결 복구로 이전 명령을 실행하거나 결과를 추정하지 않는다.
...
| 엔진룸 동물 | 별도 엔진룸 감지 입력 기능, 중앙 통합 담당 | 동물 존재/정상 미감지와 품질. 실물 감지기 구성 전에는 모의 입력. CIS 실내 관측과 분리 |
...
| 엔진룸 동물 | 별도 감지 입력의 유효 동물 존재 | 동일 감지 영역의 새 유효 정상 미감지. 센서 무응답은 정상 미감지가 아님 |
...
시작·유지·정지 요청의 권한과 필수 입력은 기능별로 정한다. 필요한 입력이 아직 정의되지 않은 자동 기능을 임의 허용하지 않는다. 이미 실행 중인 기능의 유지/정지는 기존 기능별 계약을 따르며, 이 절의 가용성 표로 일괄 OFF 처리하지 않는다.
...
|---|---|---|---|
| 공조·실내 조명 | **[잠정]** §3.2.5의 선행 예약·§3.5 피드백 구성, 기존 모드 중재 | 시각/허용 입력 구현, 회전·방열 검증, 색상·패턴 | 중앙+BCM·CIS·MOBILE |
...
| 엔진룸 동물 경고 | **[잠정]** 별도 감지 입력 기능과 발생/해제 | 실제 센서·판정 방법·정상 미감지 확인 능력 | 중앙 통합 담당; CIS 제외 |
| 요청·연결·보안 | 활성 연결·조회·로컬 등록 해제·선택 백그라운드 | 기능별 기한·식별 보관·차량 권한 폐기·경고 이력/확인 동기화. 재연결 한도는 §6.6.5의 잠정값 확인 | 중앙+MOBILE·인터페이스 |
...
> **[검토 필요]** 도어·공조 피드백은 §3.5의 잠정 구성을 따른다. 실제 확인 방법·결과 기한과 실내 조명 색상·패턴은 미정이다.
...
재전달 결과는 과거 요청 결과다. 새 현재 도어 상태 및 새 발생으로 취급하지 않는다. 정의된 한 실행 안의 제한 재시도는 동일 요청의 재전달에 의한 새 실행과 구분한다. 인증·권한 판정과 로컬 인터록은 [공통 계약](#common-request-contract)을 함께 적용한다.
...
| <a id="mb-sys-req-009"></a>MB-SYS-REQ-009 | 연결 상태가 `AUTHENTICATED`가 아니거나 대상 차량·해당 기능에 대한 유효 권한을 확인할 수 없으면 제어 요청을 생성하지 않고 사유를 표시해야 한다. 최종 수용 판단은 COM-SYS-AUT-001의 차량 측 책임을 따라야 한다. |
...
| <a id="mb-sys-alt-007"></a>MB-SYS-ALT-007 | 앱 재진입·재연결 후 권한 있는 차량에서 현재 경고와 보관된 미확인 이력을 조회해야 한다. 해제된 과거 경고는 현재 위험과 구분하고, 조회 불가·이력 누락을 경고 없음으로 표시해서는 안 된다. |
...
| <a id="mb-sys-con-002"></a>MB-SYS-CON-002 | **[잠정]** 사용자가 연결을 원하는 활성 화면에서 예기치 않은 단절이 발생하면 §6.6.5의 횟수·총 시간 한도로 자동 재연결을 시도해야 한다. 한도에 도달하면 재시도를 종료하고 사유를 표시해야 한다. 명시적 연결 해제·등록 해제·인증 실패·권한 철회·화면 비활성화 시 기본 구성의 자동 재시도를 중단해야 한다. |
...
| <a id="mb-sys-con-004"></a>MB-SYS-CON-004 | 연결 복구 후 차량 측 인증·권한을 다시 확인하고 최신 설정·상태·경고와 미확정 요청 결과를 조회해야 한다. 확인된 항목부터 갱신하며 아직 받지 못한 항목은 이전 값으로 정상 복원하지 않아야 한다. |
...
| <a id="mb-sys-sec-005"></a>MB-SYS-SEC-005 | 해당 차량의 단말 등록 해제가 확정되면 보관 인증 정보를 삭제하고 연결·자동 재연결·새 제어 요청을 중단해야 한다. 단말 삭제만으로 차량 측 권한 폐기를 완료했다고 표시해서는 안 되며 재등록 시 새 인증을 거쳐야 한다. |
...
| 알림/결과 | 현재 경고·과거 미확인 이력·최근 요청 결과 | 경고 확인 |
...
재연결 후 새 인증 세션에서 과거 결과를 조회할 수 있다. 이때 차량이 현재 조회 권한과 원 차량·원 세션·원 요청의 연결을 확인한 응답만 사용한다. 새 인증만으로 과거 세션의 지연 패킷을 직접 수용하지 않는다. 원 요청의 근거가 없으면 UNKNOWN을 유지한다.
...
| 재연결 | 차량 인증·권한 확인 → 최신 설정/상태/경고·미확정 결과 조회. 확인된 항목부터 표시 |
| 활성 화면의 예기치 않은 단절 | **[잠정]** 자동 재연결 최대 5회 또는 총 60 s 중 먼저 도달한 한도에서 종료. 간격은 1 s부터 2배 증가, 30 s 상한 후보 |
| 연결 해제·인증 실패·등록 해제·권한 철회 | 자동 재연결과 새 제어 중단. 사용자가 원인을 해결하고 다시 연결해야 함 |
...
| 로컬 차량 등록 해제 | 해당 인증 정보 삭제. 오프라인에서도 수행하되 차량 권한 폐기 완료로 표시하지 않음 |
...
차량 등록 해제 때 화면의 해당 차량 현재 상태와 요청 대기 항목도 정리하고 새 요청을 보내지 않는다. 과거 결과 이력의 보존 범위는 별도 정책이며 재등록 후 현재 상태로 재사용하지 않는다. 차량 권한 폐기 절차·전달 보장은 별도 미결이다.
...
중앙은 현재 경고와 보관된 발생 이력을 권한 있는 사용자에게 연결한다. 현재 위험, 마지막 유효 위험 이력, 정보 품질, 사용자 확인은 별도다. 같은 발생의 반복 전달은 새 경고가 아니며, 새 발생은 이전 경고를 읽었어도 미확인으로 표시한다. 확인 정보 전달과 차량 측 경고 이력 보존 범위·기간은 인터페이스 단계에서 정한다.
...
**[잠정]** 이탈 후 열린 도어·엔진룸 동물 경고의 입력과 발생·해제는 [§3.2.3~3.2.4](#central-input-profile)를 따른다. MOBILE은 모의 경고와 실제 관측 경고를 구분하고 감지 미지원·확인 불가를 정상으로 표시하지 않는다. CIS 탑승자 존재나 앱 단절을 이탈·엔진룸 감지로 대신하지 않는다. 후방 경고를 새 모바일 필수 화면으로 추가하지 않는다.
...
| <a id="mb-sys-int-011"></a>MB-SYS-INT-011 | **[잠정]** 권한 있는 대상 차량의 경고 종류·발생 식별·등급·현재 위험 상태·품질·실측/모의 구분과 미확인 이력을 제공받아야 한다. 앱 확인이나 수신 중단을 위험 해제로 해석해서는 안 되며, 모의 경고는 시연임을 식별 가능하게 표시해야 한다. |
...
| ID | 요구사항 |
|---|---|
| <a id="mb-sys-int-014"></a>MB-SYS-INT-014 | 통합 SR §7.3.1의 도어 목표, 목표 온도·자동 사용, 수동 순환, 예정 시각·선행 사용/정지, 조명 사용·밝기·색상 중 요청 대상과 값을 명확히 제공해야 한다. |
...
| Unrelated stale input | 유효 권한+무관 센서 STALE만으로 전체 제어를 차단하지 않음; 기능별 필수 입력·로컬 보호는 검사 |
...
| Cross-session query | 재인증 후 권한 있는 조회로 원 요청 확인. 과거 세션 지연 패킷은 직접 수용하지 않음 |
| Reconnect stop | 화면 비활성·명시적 해제·인증 실패·권한 철회 시 자동 시도 중단 |
| Local unregister | 오프라인에서도 해당 자격정보 삭제·재연결 중단. 차량 권한 폐기 완료로 표시하지 않음 |
...
| 요청 | 결과 보관 건수 | 50 | 최근 요청 결과의 보관 한도 후보 | 사용성 검토 |
...
| 사용자 STOP·수동 해제·정상 대체·명시적 권한 철회 | CANCELLED | 목표 완료와 구분 |
...
| <a id="win-sys-saf-003"></a>`WIN-SYS-SAF-003` | ECU는 진행 중인 상위 요청의 만료·권한 철회·유효성 상실 또는 통신 상실 시 그 상위 이동을 정지해야 한다. 무관한 새 요청의 오류만으로 기존 정상 동작을 중단하지 않아야 한다. 독립 로컬 조작과 끼임 보호는 §8.6의 조건을 따라야 한다. | — |
...
| 엔진룸 동물 경고 | 별도 감지 입력 기능→중앙, CIS 제외 | MOBILE | 현재 경고·품질·모의 여부 | **[잠정]** 유효 존재/정상 미감지. 실제 센서 미구성은 감지 미지원 |
| 사용자 인증/권한 | 중앙의 차량 측 권한 판단 기능; 물리 배치 미정 | MOBILE·중앙 요청 수용 | 인증 결과·차량별 권한·유효 범위 | VSS 후속 ECU 보안과 별개 |
...
> **[검토 필요]** 도어는 독립 접점의 잠정 구성을 따른다. 실제 센서·상태 확인 방법·결과 확인 기한·완료음 전달 기한은 검증이 필요하다. 선행 공조는 §3.2.5의 잠정 조건을 따른다. 실제 시각·허용 입력과 방열 피드백 검증은 필요하다. 지원 색상·패턴과 밝기의 실제 표현·검증 조건은 미정이다.
...
안전 알림은 조도 기반 NORMAL 밝기를 그대로 사용하지 않는다. 점멸 등 알림별 표현으로 구분하며, NORMAL이 이미 최대 밝기일 때 불가능한 추가 밝기 차이를 요구하지 않는다. 구체 색상·패턴·주기와 지원 밝기 변환은 후속 설계에서 정한다.
...
| 인증 절차 응답 | 사용자 인증 | 중앙 판단 기능 | 미정(TBD) |
...
> **[검토 필요]** 지원 색상·패턴과 밝기의 실제 표현·검증 조건은 미정이다. 실제 정보 전달 형식과 일부 입력 공급원은 미정이다.
...
| 경고 종류·발생 식별·등급·현재 위험·품질·실측/모의 구분·미확인 이력 | 권한 있는 사용자 표시; READ와 CLEAR 및 모의 시연 구분 | 중앙 (원본 의미/이력 연결) | 미정(TBD) |
