...
버전: v0.22 · 상태: 공유용 초안 · 수정일: 2026-09-17
...
아래는 기능별 대상과 요구 정의 수다. 이번 단말 등록·디지털 키의 담당은 §2.1.1·§3.2.7을 따르며, 그 밖의 물리 배치·통신 상세는 각 절의 기존 확인 항목을 따른다.

| 영역 | 대상 | 요구 정의 수 |
|---|---|---:|
| BCM | 전용 S32K144 ECU | 88 |
| CIS | 전용 처리 보드 | 59 |
| MOBILE | Android 애플리케이션 | 103 |
| VSS | 전용 S32K344 ECU | 114 |
| WINDOW | 전용 S32K144 Window ECU | 73 |
| EXTERIOR_LIGHT | S32K144 기능 요소 후보, 실제 배치 미정 | 58 |
| 영역별 소계 | 위 6개 영역 | 495 |
| 공통 | 요청·등록 확인·품질·범위·중앙 책임·감시·디지털 키 | 16 |
| 전체 | 후속 보안 후보 제외 | 511 |
...
| 단발성 이벤트·음향(One-shot) | 한 번의 발생을 기준으로 처리하는 단발성 이벤트 또는 음향 |
| 지속형 정보(Stateful) | 현재 의미 상태와 갱신 품질을 계속 확인하는 지속형 정보 |
| 선택된 재생 대상(Winner) | 현재 중재 결과 선택된 재생 대상 |
| Sound Asset | 저장된 음원 자산 |
| Playback Session | 음향 재생 시작부터 종료까지의 재생 세션. 패턴에 포함된 무음 구간도 포함 |
| Hold | 입력을 신뢰할 수 없을 때 마지막 유효 의미를 제한된 시간 유지하는 처리 |
...
| SNA | 정보 제공 기능이 정상 의미 상태를 제공할 수 없는 상태. 정상 CLEAR와 구분 |
| Availability | 기능·출력을 제공할 수 있는 범위. VSS의 입력 수용 가능 여부와는 구분 |
| `Semantic Owner` | 정보의 의미를 판단하는 담당 |
| Network Producer / Consumer | 물리적으로 정보를 전달하는 노드 / 그 정보를 사용하는 노드 |
...
| <a id="com-sys-aut-002"></a>COM-SYS-AUT-002 | 차량 출처·인증/세션 유효성과 개별 상태의 품질을 별도로 판단해야 한다. 출처 또는 인증/세션이 확인되지 않으면 MOBILE의 새 제어 요청을 차단해야 한다. 유효 세션에서 무관한 상태 하나의 STALE/INVALID만으로 모든 제어를 차단해서는 안 된다. 중앙은 기능별 필수 입력의 유효성·최신성·허용 조건을 확인해야 하고, 실행 노드는 로컬 인터록과 보호를 유지해야 한다. 필수 입력 또는 허용 조건이 정의되지 않은 기능을 임의 허용하지 않아야 한다. 정지 요청의 필수 조건은 해당 기능 계약으로 정하며 모든 품질 검사나 인증을 일괄 면제하지 않아야 한다. |
...
| <a id="com-sys-evt-001"></a>COM-SYS-EVT-001 | 중앙의 도어 의미 판단 기능은 유효한 요청 실행과 BCM의 확인 결과를 연결하여 실제 목표 잠금 전이가 확인된 경우에만 DOOR_LOCK_COMPLETE 또는 DOOR_UNLOCK_COMPLETE 이벤트를 생성해야 한다. 정상 상태에서 이미 목표가 충족된 무구동 DONE, 같은 요청의 재전달, REJECTED, UNKNOWN만으로 새 완료 이벤트를 생성해서는 안 된다. 수용 후 실제 LOCK 실행의 목표 확인 실패(NO_FEEDBACK 또는 DRIVE_LIMIT_EXCEEDED)는 DOOR_LOCK_ERROR로 연결해야 하며 잠금 해제 실패를 이 이름으로 변환해서는 안 된다. 각 발생은 원 요청·대상 도어와 종류에 연결된 동일 발생 식별을 유지하여 중계·재전송 시 중복 재생되지 않아야 한다. VSS 재생은 기존 유효성·나이·중재 계약을 따라야 한다. |
...
| REJECTED | 실행 시작 전에 실행 허용·유효성·인터록 등으로 거부됨. 중앙 수용 뒤 로컬 실행 전 거부도 포함 |
...
| 도어 사례 | 결과 | 새 도어 발생 |
|---|---|---|
| 유효한 새 요청·안전 조건 충족·NORMAL·이미 목표 상태 | DONE + ALREADY_AT_TARGET, 무구동 | 없음 |
...
무구동 완료는 새 요청을 판정한 시점의 유효한 현재 상태에 근거한다. 과거 중복 요청에 반환된 DONE은 과거 요청 결과이며 현재 도어 상태의 최신 증거가 아니다. 물리 완료의 정의와 VSS 실제 음향의 중재·전달 시간 충족은 별도다.
...
| 도어 발생 | 중앙 의미 소유, BCM 실행 근거, VSS 소비. **[잠정]** 음향 시작 나이는 §7.6.3의 2초 미만 | 발생 식별·나이 전달 방식·중재/실제 인지 기한 검증 필요 |
...
|---|---:|---|
| MOBILE → 중앙: 목표 온도·자동 공조 사용·선행 예약·실내 조명 설정 | 2 s | 중앙 설정 반영. 공조 실행·도달 온도·실제 점등과 별도 |
| MOBILE/중앙의 도어 잠금·해제 요청 관측 | 3 s | 실제 목표 잠금 또는 정상 무구동 목표 충족 |
| MOBILE/중앙의 수동 Fan·공조 실행/정지·실행 중 선행 공조 취소 관측 | 5 s | 해당 요청에서 요구한 Fan/출력 상태·종료 확인. 목표 온도 도달은 제외 |
| 대기 중 선행 예약 취소: MOBILE → 중앙 | 2 s | 예약 취소 확인. 실행 중이면 위 5초 적용 |
...
각 결과 확인 기능은 자신이 관측한 접수 시점을 기준으로 감시하므로 이 표의 시간을 종단 전체 성능 보장으로 합산하거나 등치하지 않는다. 중간 응답·유지 허용 갱신으로 대기를 다시 시작하지 않으며, UNKNOWN 뒤에는 확인된 최종 결과만으로 해소한다. UNKNOWN만으로 자동 재실행하거나 성공/실패를 추정하지 않는다.

예약의 실제 시작까지의 대기, 선행 공조·환기의 최대 5분, 외부 임시 점등 10초는 작업 상태로 따로 감시한다. 모바일의 WINDOW/ELS 제어 범위를 추가하지 않는다. 로컬 구동 상한·재시도·확인/보호 기한은 기능별 후보 또는 확인 필요 조건을 유지한다. 필요한 로컬 상한·피드백이 정해지지 않은 실구동은 그 조건을 확인하기 전에 활성화하지 않는다.
...
| <a id="com-sys-ctr-001"></a>COM-SYS-CTR-001 | **[잠정]** 중앙은 목표 온도·자동 공조 사용·공기 순환 선택·선행 공조 예약/사용·실내 조명 사용/밝기/색상의 확정 설정과 자동 기능 작업 상태를 관리해야 한다. MOBILE은 요청값을 생성하고 중앙이 확인한 설정을 표시해야 하며, BCM은 중앙의 실행 명령과 실제 적용 상태·결과를 제공해야 한다. 중앙은 동일 자원에 대한 수동·자동·알림 요청을 중재해 일관된 실행 목표를 내려야 하고, 일시적인 출력 개입을 사용자 설정의 변경으로 오인해서는 안 된다. 선행 공조·자동 환기는 §3.2.5~3.2.6, 지원 구성은 §3.5를 따라야 하며, 기능별 우선순위·로컬 보호와 미지원 제한을 지켜야 한다. 외부 자동 점등은 중앙이 §9.6.2.1의 잠정 조건으로 판단해야 한다. |
| <a id="com-sys-ctr-002"></a>COM-SYS-CTR-002 | **[잠정]** 중앙은 §3.2.3~3.2.4의 입력 계약과 발생·해제 조건에 따라 차량 사용 의미와 잔류 탑승자·끼임·이탈 후 열린 도어 경고를 판단해야 한다. 끼임 감지·즉시 보호·실제 반전 결과·현장 확인 전달은 WINDOW가, 탑승자 관측과 품질은 CIS가 제공해야 한다. 후방 근접 위험의 의미 판단은 CIS가 담당하며 중앙은 이를 중복 판정해서는 안 된다. 각 경고의 상태·품질·모의 여부를 구분하고, 앱 확인·미수신·음향 종료만으로 위험 해제를 생성해서는 안 된다. 실제 입력이 미구성인 기능의 모의 시연을 실제 감지 지원으로 표시해서는 안 된다. |
...
| <a id="com-sys-mon-001"></a>COM-SYS-MON-001 | **[잠정]** 정보를 사용하는 기능은 주기 제공이 약속된 정보와 발생 시 제공되는 이벤트·명령을 구분하여 감시해야 한다. 주기 정보는 감시 활성 조건·첫 수신 대기 기준·최신성 기한·복구 조건을 정한 뒤 감시해야 한다. 이벤트·명령의 단순 미발생은 통신 고장 근거로 사용해서는 안 되며, 발생 또는 요청이 확인된 뒤에는 해당 전달·응답 기한을 감시해야 한다. 통신 정상 여부는 정해진 주기 정보나 연결 진단 근거로 판단하고, 근거가 없는 상태를 정상으로 추정해서는 안 된다. 이 규칙은 기능별 통신 상실 출력 정책을 변경하지 않는다. §2.5의 상태 제공과 §2.6의 활성 목표 유지 허용을 구분해야 한다. 각각의 주기·나이 기준을 적용하고 명령 재전달로 기한을 연장해서는 안 된다. |

<a id="common-freshness-profile"></a>
### 2.5 상태 최신성과 주기 감시

**[잠정]** 아래는 현재 시연의 논리 상태 제공 기준이다. `P`는 차량 내부에서 새 관측 또는 현재 품질을 제공하는 주기, 최대 나이는 최종 소비 시점의 원본 정보 나이 한도다. MOBILE로 전달하는 무선 구간의 주기는 아래 별도 기준을 적용한다. 내부 센싱 주기와 네트워크 제공 주기는 별도로 충족해야 하며, 상태·품질 변화의 더 짧은 제공 기한이 있으면 먼저 적용한다.

| 정보·경로 | 주기 P | 원본 최대 나이 |
|---|---:|---:|
| 후방 위험·활성·품질: CIS → 중앙/VSS, 중앙 → CIS의 후방 활성 허용 | 200 ms | 500 ms |
...
최대 나이에 **도달하면** STALE로 본다. INVALID/SNA·허용 정보를 신뢰할 수 없는 상태는 나이 한도를 기다리지 않고 즉시 반영한다. 복합 경고는 판정에 필요한 각 입력의 품질·나이 한도와 경고 자체의 한도를 모두 충족해야 한다. 중앙이 같은 옛 입력을 재계산하거나 중계가 다시 보냈다는 이유로 원본 정보 경과 시간을 초기화하지 않는다. 전달 경로 전체의 경과 시간과 시각 오차를 포함한 나이 근거를 확인할 수 없으면 유효로 취급하지 않는다. 끼임의 미해제 발생 이력과 그 이력을 확인해 제공하는 현재 상태의 시점은 구분한다.

**[잠정]** 지정된 주기 경로의 수신 허용 지연은 100 ms다. 송수신 준비·주기 제공 약속이 활성화된 시점을 기준으로 첫 예정 수신 기한은 `P+100 ms`, 이후 기한은 P 간격이다. 기한까지 도착한 새 유효 통신 갱신을 먼저 반영한다. CIS 관련 경로의 연속 누락 3회 판정은 §5.7.1을 따르며, 실제 주기/위상을 반영한 예정 기한을 사용한다. 예를 들어 기준 시각 0의 P=200 ms 경로가 계속 미수신이면 300/500/700 ms에서 누락을 계수한다. 처음부터 데이터가 없으면 NO_DATA이며 통신 오류 확정 전에도 정상값을 만들지 않는다. 진단 메시지 수신은 센서 원본 정보 경과 시간을 갱신하지 않는다.

**[잠정]** MOBILE의 일반 상태 제공 주기는 500 ms, 무선 구간의 마지막 새 갱신 수신 후 경과 시간 한도는 2초다. 항목별 원본 최대 나이도 함께 적용해 먼저 만료된 근거를 표시한다. 위 표에 없는 항목의 원본 기한은 별도이며 2초를 공통 구동 허용으로 사용하지 않는다. 경고·품질·요청 결과의 변화 전달과 표시에는 기존의 더 짧은 기한을 유지한다. 같은 내용이어도 새 관측/품질 평가임을 확인할 수 있어야 하며 단순 패킷 재전달은 수신 후 경과 시간을 새로 시작하지 않는다.
...
**[잠정]** 새 요청의 수용 기한과 수용 후 목표의 유지 허용을 분리한다. 수용 기한은 아래 출발 시점부터 단조 증가 시간으로 계산한 원본 정보 경과 시간이 한도 미만일 때만 유효하다. 전달·대기 시간과 시각 오차를 포함해 남은 기한을 확인할 수 없으면 새 실행을 거부한다. 실제 전달 필드와 시간 표현은 인터페이스 설계에서 정한다.
...
같은 요청의 재전달은 원본 기한을 갱신하지 않는다. 이미 수용·완료된 요청은 기존 상태/결과만 반환하며 다시 실행하지 않는다. 수용 후에는 아래 유지 허용과 해당 작업·로컬 상한을 적용한다. 수용 기한 1초를 진행 중 이동 전체의 1초 제한으로 사용하지 않는다. 예약을 수용한 뒤에는 중앙이 실제 시작 조건을 새로 판단해 실행 명령을 만들며 오래된 예약 요청을 실행 명령으로 재전달하지 않는다.

**[잠정]** 중앙이 선택한 실행 예정·활성 출력 목표의 유지 허용은 100ms마다 새로 평가해 제공한다. 실행 노드는 마지막 유효 평가 시점부터 300ms가 경과하면 해당 유지 허용을 상실한 것으로 처리한다. 갱신 지연은 이 300ms 안에 포함하며 추가 유예를 붙이지 않는다. 명시적 철회·INVALID는 기한까지 기다리지 않고 반영한다. 초기 허용은 없음이며, 첫 실행에는 현재 대상·목표와 연결된 유효한 허용이 필요하다.

유지 허용은 새 실행 명령과 별도의 논리 상태다. 대상 기능·현재 목표/작업 식별·새 평가 순서·나이·허용 여부를 구분한다. 중앙은 현재 요청 출처·실행 허용·필수 입력·실구동 허용을 확인할 수 있을 때만 갱신한다. 같은 평가의 재전달, 무관한 상태 수신 또는 다른 목표의 허용으로 현재 목표를 연장하지 않는다. 갱신은 구동 재시작·재시도 횟수·작업 타이머·결과 대기를 초기화하지 않는다. 입력 근거 자체의 나이도 보존하며 모의 정보로 실제 구동 허용을 만들지 않는다.
...
이 감시는 중앙이 선택한 실행 예정·활성 상위 출력 목표에만 적용한다. 출력이 없는 정상 대기·저장된 설정·대기 중 예약에는 지속 실행 명령을 요구하지 않는다. 앱 연결 단절은 실행 허용 철회와 구분하며 중앙의 유지 허용을 앱의 주기 메시지로 대신하지 않는다. ELS 임시 점등의 10초와 선행/환기의 5분 상한은 허용 갱신으로 늘어나지 않는다.
...
| 영역 | 담당 책임 | 관련 영역의 책임 |
|---|---|---|
| 중앙 | 단말 등록 확인·확정 설정, 자동 기능·자원 중재, 차량 사용 의미·잔류 위험·끼임 경고·이탈 후 도어 경고의 판단, 결과 연결·사용자 표시용 상태 매핑 | 직접 센서 측정·로컬 끼임/전기 보호·음향 내부 재생 중재 |
| BCM | 도어·공조·실내 조명의 명령 실행, 직접 관측 가능한 적용 상태·결과·자체 보호 | 사용자 설정의 최종 소유·자동 공조 정책·센싱 정보 중계 |
| CIS | 실내 관측·품질, 후방 거리의 내부 측정과 위험 의미 판단 | 잔류 위험의 차량 수준 판단·음향 출력 |
...
|---|---|---|---|---|
| <a id="central-c01"></a>도어 | MOBILE 요청, BCM 잠금·개폐·품질 | 중앙 등록 확인·목표/결과 연결 | BCM 실행; MOBILE 표시; VSS 도어 발생 소비 | 도어 결과·잠금/개폐는 구분 표시. 무구동 완료 시 새 완료음 없음. 결과 대기는 §2.3.1의 잠정 3초. 물리 전달·기한 충족 미검증. |
...
| <a id="central-c03"></a>선행 공조 | MOBILE 예약, 중앙 시각, CIS 온도, 실제 구동 허용·BCM 상태 | 중앙 1회 예약·시작/종료·공조 목표 | BCM 실행; MOBILE 작업 표시 | **[잠정]** §3.2.5의 예정 5분 전·최대 5분. 조건 미충족/실패 후 자동 재개 없음. |
...
| <a id="central-c05"></a>실내 관측 | CIS 탑승자·온도·습도·조도·품질 | CIS 측정/관측 확정; 중앙 기능별 활용 | 중앙; MOBILE 현행 표시 항목 | 값과 나이·품질 유지. 부분 고장·회복 §5.3/5.10 반영; 모든 값을 앱 필수 표시로 확대하지 않음 |
| <a id="central-c06"></a>후방 경고 | CIS 거리·품질(내부) | CIS 위험 의미 판정, 중앙 중계 시 의미 보존 | VSS 후방 경고 | CIS 유효 해제·품질. 원시 거리·임계값 운영 외부 제공 제외. 경계/신뢰할 수 없는 상태는 §5.6, 음향 출력은 §7.6을 따름 |
...
| <a id="central-c08"></a>자동 환기 | 현장 사용 허용, 사용 상태, CIS 온도·존재, WINDOW 위치·보호 | 중앙 온도 조건·1회 환기·취소 | WINDOW 목표 실행 | **[잠정]** §3.2.6의 30/28°C·위치 80%·최대 5분. 종료·중단 후 자동 닫힘 없음. |
...
| <a id="central-c11"></a>차량 사용 음향 | 차량 상태 입력 기능의 사용 상태·품질 | 중앙 사용 시작/종료 의미 확정 | VSS; 선택 조명은 별도 계약 | **[잠정]** §3.2.3의 유효 전이만 발생 생성. 첫 상태·Wake·재접속은 새 발생 아님. 음향 시작 나이는 §7.6.3의 2초 미만. 실제 나이 전달·전원 유지 미검증. |
...
| <a id="central-c17"></a>디지털 키 | MOBILE 사용 설정; ESP32 등록·연결·근접·품질; BCM 도어 상태 | 중앙 새 접근·실행 허용·해제 목표 | BCM 실행; MOBILE 원인·결과 표시; 기존 VSS 도어 발생 | **[잠정]** §3.2.7의 활성 화면·초기 OFF·비근접 후 새 접근 조건. 실제 근접 기준·주기·유효 시간 확인 필요. |
...
| 상황 | 기본 처리 |
|---|---|
| 사용자 수동 순환 선택 | 수동 Fan을 적용하고 자동/선행 온도 제어를 종료. 온도 장치는 OFF |
| 사용자 자동 공조 선택 | 현재 유효 온도와 목표로 Fan·온도 목표를 계산. 기존 수동/선행 동작은 대체 |
| 목표 온도만 변경 | 설정을 갱신. 활성 자동/선행 모드에 반영하되 꺼진 공조를 새로 시작하지 않음 |
| 선행 공조 예약 | **[잠정]** 설정 수용과 실제 시작을 구분. §3.2.5의 시각·허용·공조/환기 조건에서 1회 시작 |
| 선행 공조 중 사용자 조작 | 새로 선택한 수동/자동 모드가 대체. 종료된 선행 작업을 자동 재개하지 않음 |
| 선행 공조 정지 | 해당 예약/작업 취소. 다른 모드의 현재 동작을 끄지 않음 |
| 탑승 예정 시각 또는 시작 후 5분 도달 | **[잠정]** 선행 작업 종료, 해당 Fan·온도 목표 OFF. 목표 온도 도달만으로 작업 종료로 보지 않음 |
| 자동/선행 제어의 필수 온도·허용 입력 상실 | 해당 자동 목표 OFF·사유 제공. 선행 작업은 실패로 종료. 무관한 수동 순환까지 차단하지 않음 |

일반 공조는 가장 최근에 명시적으로 선택한 모드를 사용한다. 자동 사용 해제는 해당 자동 모드만 종료한다. 대체·종료된 모드는 이전 목표나 입력 복구만으로 다시 시작하지 않는다. 선행 예약의 시각·1회 실행 조건은 §3.2.5를 따른다. §3.2.3의 모의 사용 상태만으로 실구동을 허용하지 않으며, 놓친 시작을 입력 복구 뒤 실행하지 않는다.
...
|---|---|---|
| 도어 잠금·해제 | BCM의 잠금·개폐·품질·로컬 인터록. LOCK은 열린 도어 금지 | BCM 목표 확인 또는 정상 무구동 완료. 실제 열림 상태와 새 완료음은 별도 |
| 목표 온도·자동 공조 사용 | 설정은 지원 범위 확인. 실제 자동 시작은 유효 CIS 온도·BCM 상태·전원/허용 필요 | 중앙의 설정 반영 확인. 실제 모드·Fan·온도 출력·고장 및 목표 온도 도달과 구분 |
| 수동 공기 순환 선택 | 지원 수준·BCM Fan 상태·허용. 자동 온도 입력 품질은 전역 차단 사유 아님 | 중앙 모드 선택과 BCM의 해당 Fan 목표 확인. 이전 자동/선행 모드는 §3.2.1대로 종료 |
| 예정 시각·선행 공조 사용 | **[잠정]** 날짜·시간대가 포함된 시각, 최소 5분 뒤의 예약. 실행 조건은 §3.2.5 | 중앙 예약 설정 반영. 예약·진행·종료·취소·실패와 사유는 별도 작업 상태 |
| 선행 공조 정지·사용 해제 | 해당 예약/작업 식별과 현재 모드. 정상 온도값은 정지 필수 입력 아님 | 예약 취소 또는 해당 작업 종료와 필요한 BCM OFF 확인. 이미 다른 모드로 대체됐다면 그 모드를 끄지 않음 |
| 실내 조명 사용·밝기·색상 | 지원 설정 범위·중앙 알림 중재. 자동 밝기에만 유효 실내 조도 필요 | 중앙 설정 반영 확인. BCM 적용 결과·현재 알림·밝기·색상은 별도; 안전 알림 개입 시 사유 표시 |
| 근접 자동 잠금 해제 사용 여부 | 등록된 단말·현재 연결, §3.2.7의 근접 판단 지원 여부 | 중앙의 사용 설정 반영. 설정 DONE과 실제 도어 해제는 별개 |

설정 요청의 DONE은 중앙이 반영값을 확정해 반환한 때다. 차량 재시작 뒤에도 보존한다는 뜻은 아니며 저장 지속성은 후속 결정이다. 미지원 값·해석 불가 시각·미정 정책으로 실행 보장할 수 없는 사용 설정은 거부하거나 제한 사유를 명시하며 정상 활성 상태로 꾸미지 않는다. 실행 조건이 아직 충족되지 않은 예약 설정을 보관한 경우에는 ‘예약됨/실행 조건 미충족’을 구분해 제공한다.
...
차량 사용 시작은 유효한 ‘사용 종료→사용 중’, 사용 종료는 유효한 ‘사용 중→사용 종료’ 전이로만 생성한다. 첫 유효 상태와 품질 회복 후 첫 상태는 현재 상태 동기화이며 과거 전이를 추정하지 않는다. 동일 발생의 재전달은 식별을 유지한다. 전원 인가·Wake·앱 재접속·선행 공조 시작으로 차량 사용 발생을 만들지 않는다. 사용 상태는 전원/구동 허용 신호와도 구분한다.
...
끼임 확인은 해당 발생·대상 채널과 연결된 새로운 현장 입력이어야 한다. 계속 눌린 입력·이전 발생의 확인·앱 읽음·반전 완료·STOP만으로 해제하지 않는다. WINDOW는 미해제 발생과 수용한 현장 확인을 상태로 제공해 이벤트 유실 후에도 중앙이 재동기화할 수 있어야 한다. 중앙은 같은 발생의 유효 해제를 기억하고 이전 이벤트의 재전달이나 후속 일반 이동만으로 이를 다시 활성화하지 않는다. 새 끼임은 이전 확인을 무효화하며, 확인과 새 끼임이 겹치면 새 끼임을 우선한다. 최초 기동/기억 상실로 해제 여부를 모르면 확인 불가이며, 출력·이동 정지와 보호 처리 종료를 확인한 뒤 새 현장 확인으로 초기 CLEAR를 정할 수 있다. 경고 CLEAR는 로컬 고장 해제나 이동 재개 명령이 아니다.
...
<a id="central-preclimate"></a>
### 3.2.5 선행 공조 예약

**[잠정]** 예약은 1건·1회 실행이며 `시작 시각 = 탑승 예정 시각 − 5분`, 최대 실행 시간은 5분으로 정한다. 수용 시 예정 시각은 중앙 현재 시각보다 최소 5분 뒤여야 한다. 날짜·시간대를 포함해 비교하고, 중앙 시각이 유효하지 않으면 예약을 거부한다. 새 예약을 먼저 검증·수용한 뒤 대기 중인 이전 예약을 대체한다. 실행 중 변경은 해당 선행 작업을 종료한 뒤 새 예약으로 처리하며, 잘못된 새 요청만으로 기존 작업을 종료하지 않는다.

| 구간 | 처리 |
|---|---|
| 시각 공급 | 중앙의 차량 시각 기능을 사용. 시연 담당자가 현장에서 시각을 설정하고 유효 상태를 명시한다. MOBILE은 예정 시각을 요청하며 중앙 시각을 임의 보정하지 않음 |
| 시작 | 유효 시각이 시작 시각에 도달한 첫 평가에서 한 번 판단. 사용 종료·유효 CIS 온도·실제 구동 허용·BCM 공조 기능의 시작 허용·로컬 보호 조건, 다른 공조 모드 및 자동 환기 작업 없음이 모두 필요 |
| 시작 조건 미충족 | 예약 작업 FAILED와 사유. 뒤늦게 조건이 좋아져도 재시작하지 않음 |
| 실행 | 최신 확정 목표 온도와 유효 실내 온도로 기존 공조 목표 산출. Fan 정상 확인 후 온도 출력 허용. 목표 온도 도달은 출력 조정이며 예약 종료가 아님 |
| 정상 종료 | 예정 시각 또는 시작 이후 단조 증가 시간으로 5분 중 먼저 도달한 때 해당 공조 목표 OFF, 작업 종료 |
| 취소 | 사용자 정지·수동/자동 모드 선택·차량 사용 재개·정상 허용 철회. 해당 작업만 종료하고 대체된 다른 모드의 출력을 끄지 않음 |
| 실패 | 필수 입력을 신뢰할 수 없음·관련 고장·시각 변경/유효성 상실. 해당 목표 OFF·FAILED와 사유. 정상 복귀만으로 재개하지 않음 |

예약 대기 중 시각이 재설정되거나 시각 정보를 신뢰할 수 없게 되면 예약을 취소하고 새 예약을 받아야 한다. 실행 중 최대 시간은 시계 보정으로 늘어나지 않아야 한다. 중앙 재기동 뒤 이전 예약을 자동 복원·실행하지 않는다. 종료된 예약은 비활성으로 표시하며 반복 예약을 자동 생성하지 않는다. 5분은 중앙의 선행 목표 유지 상한이며 실제 OFF 적용 확인 기한은 별도다. 종료 조건에서 해당 목표를 끄고, 필요한 OFF 적용을 확인한 뒤 정상 종료를 제공한다. 확인할 수 없으면 종료 확인 불가 또는 실패 사유를 표시한다. 작업 시간과 실행 명령의 응답·피드백 기한은 별개다.
...
정상 종료·취소·실패 시 해당 자동 요청이 아직 이동을 소유한 경우에만 정지를 요청한다. 이미 로컬 조작이나 새 요청으로 대체되었다면 그 동작을 별도로 중단시키지 않는다. 정지 결과를 확인하지 못하면 정지 성공으로 표시하지 않는다. 종료 뒤에는 현재 위치를 유지하고, 다시 시작하려면 현장 환기 사용을 해제한 뒤 새로 허용해야 한다. 복구·온도 재상승·재부팅만으로 새 환기를 시작하지 않는다.

선행 공조와 환기는 동시에 시작하지 않는다. 같은 평가에서 두 시작 조건이 함께 성립하면 예약된 선행 공조를 먼저 판단하고 환기는 시작하지 않는다. 이미 환기 중이면 선행 예약의 시작 조건 미충족으로 처리한다. 30/28°C·5분·위치 80%는 시연용 임시 선택이며 실제 환기 성능이나 열 위험 해소를 보장하는 값이 아니다.
...
| <a id="com-sys-dk-004"></a>COM-SYS-DK-004 | 중앙은 자동 잠금 해제의 발생 원인을 해당 도어 명령·BCM 결과에 연결하고, 실제 잠금 해제 전이가 확인된 경우에만 근접에 의한 해제 완료를 MOBILE에 제공해야 한다. 무구동 DONE과 거부·실패·결과 확인 불가를 실제 해제로 표시해서는 안 된다. 도어 완료 음향은 COM-SYS-EVT-001의 기존 발생·중복 방지 규칙을 따라야 한다. |
...
| 경고 단발성 이벤트 | 정보 제공 기능이 확인한 발생의 전달·나이·중복 근거 | 수신자가 발생 사실도 모르는 상태에서 ‘경고 미발생’을 통신 고장으로 판단하지 않음 |
| 지속 위험 상태 | 원본 의미와 갱신/최신성·품질. 신뢰할 수 없을 때의 기능별 정책 | 수신 중단을 CLEAR로 치환하지 않음 |

§2.5에 지정된 상태는 잠정 주기·나이·첫 수신 기준을 적용한다. 활성 상위 목표의 유지 허용은 §2.6을 따른다. 나머지 경로·실물 입력의 유효성 기준과 실제 전달 방식은 확인이 필요하다. CIS의 누락 횟수 경계와 센서 첫 데이터 대기는 §5.3/5.7을 따른다.
...
|---|---|---|---|
| 공조·실내 조명 | **[잠정]** §3.2.5의 선행 예약·§3.5 피드백 구성, 기존 모드 중재 | 시각/허용 입력 구현, 회전·방열 검증, 색상 실측·패턴 | 중앙+BCM·CIS·MOBILE |
| 후방 위험 | CIS 의미·품질, VSS 재생과 신뢰할 수 없는 입력 처리 | 센서 능력, 전원/선택 기어 입력, 거리·최신성·Hold 실측 | CIS+VSS·인터페이스 |
...
| 차량 사용 의미 | **[잠정]** 차량 상태 입력의 유효 전이 | 실물 입력 구성, Wake/Goodbye 발생 나이·전원 유지 | 중앙+VSS·인터페이스 |
...
| 디지털 키 | **[잠정]** §3.2.7의 ESP32 근접 정보·중앙 판단·BCM 실행·MOBILE 표시 | 실제 근접 기준·입력 주기/유효 시간·앱 활성 확인·등록 해제 지원 | ESP32+중앙+BCM+MOBILE |
...
| 항목 | 기본 구성 | 지원 한계 |
|---|---|---|
| 자동 기능 구동 허용 | 현장 실행 허용을 제공하는 로컬 입력과 노드별 전원/보호 상태. 중앙은 현재 허용·품질을 확인하고 실행 노드는 로컬 조건을 다시 확인 | 초기화·리셋·입력을 신뢰할 수 없는 경우에는 불허. 새 현장 허용 필요. 모의 차량 상태·시각·경고를 이 허용이나 로컬 보호 해제의 대용으로 사용하지 않음 |
| 도어 피드백 | 잠금과 개폐에 각각 독립된 접점 입력 | 명령 기억으로 잠금/닫힘을 추정하지 않음. 실제 센서 부품·고장 검출은 미정 |
...
> **[검토 필요]** 도어·공조 피드백은 §3.5의 잠정 구성을 따른다. 실제 확인 방법·결과 기한은 검증이 필요하다. 조명 색상은 [§4.13.6](#bcm-light-colors)를 따르며, 알림 패턴·유지 시간은 미정이다.
...
| <a id="bcm-sys-dl-002"></a>BCM-SYS-DL-002 | 명령 유효성·로컬 안전 조건이 충족되고 도어 종합 상태가 `NORMAL`이며 목표 잠금 상태가 현재 잠금 상태와 동일한 경우 액추에이터를 구동하지 않고 `DONE` + `ALREADY_AT_TARGET`으로 처리해야 한다. 이 결과를 새 잠금·해제 발생으로 제공해서는 안 된다. |
...
| <a id="bcm-sys-dl-007"></a>BCM-SYS-DL-007 | 실제 도어 구동이 수행된 요청은 구동 후 정의된 확인 제한시간 이내에 목표 잠금 상태 도달을 측정으로 확인한 경우에만 정상 완료로 처리해야 한다. 무구동 목표 충족은 BCM-SYS-DL-002의 유효한 현재 상태 확인을 따라야 한다. |
...
> **[검토 필요]** 도어는 독립 접점의 잠정 구성을 따른다. 실제 센서·상태 확인 방법·결과 확인 기한·완료음 전달 기한은 검증이 필요하다.
...
| 3 | LOCK 요청 시 개폐가 CLOSED 아님 | 로컬 인터록에 의해 REJECTED + DOOR_OPEN |
| 4 | 안전 조건 충족·NORMAL·현재가 목표와 같음 | 무구동 DONE + ALREADY_AT_TARGET |
| 5 | 실제 구동 필요·허용 조건 충족 | 구동 및 안전 차단 타이머, 이후 접점 확인 |
| 6 | 확인 제한시간 내 목표 도달 확인 | DONE; 중앙에서 실제 전이 발생으로 연결 |
| 7 | 목표 미확인 | 기존 제한 재시도·보호 조건 적용 후 FAILED + 사유; 실제 LOCK 실패만 LOCK_ERROR 발생 근거 |

거부 사유 후보: `DOOR_OPEN` / `STATE_UNTRUSTED` / `CMD_INVALID`

무구동 완료 사유: `ALREADY_AT_TARGET` (거부 사유가 아님)

실패 사유 후보: `NO_FEEDBACK` / `DRIVE_LIMIT_EXCEEDED`

재전달 결과는 과거 요청 결과다. 새 현재 도어 상태 및 새 발생으로 취급하지 않는다. 정의된 한 실행 안의 제한 재시도는 동일 요청의 재전달에 의한 새 실행과 구분한다. 등록·연결 확인 및 실행 허용 판정과 로컬 인터록은 [공통 계약](#common-request-contract)을 함께 적용한다.
...
> **[검토 필요]** 지정된 상태의 주기·나이는 §2.5의 잠정값을 따른다. 나머지 경로·실제 전달/나이 표현과 실측 검증은 남아 있다.
...
| <a id="bcm-sys-perf-002"></a>BCM-SYS-PERF-002 | 유효하게 수용되어 실제 구동이 필요한 명령은 정의된 시간 이내에 구동이 시작되어야 한다. 무구동 목표 충족, 중복 요청의 기존 결과 반환 및 거부에는 새 구동 시작을 요구하지 않아야 한다. |
...
> **[검토 필요]** 보호 해제와 재동작의 실제 확인 방법·시간은 미정이다. 상태·활성 목표의 감시는 SysRS §2.5~2.6의 잠정안을 따른다. 실물 입력·복구·차단 확인과 나머지 경로는 확인이 필요하다.
...
> **[검토 필요]** 실제 정보 전달 형식과 일부 입력 공급원은 미정이다.
...
| ID | 요구사항 |
|---|---|
| <a id="bcm-sys-nfr-004"></a>BCM-SYS-NFR-004 | 동일한 유효 명령, 현재 상태·이력과 로컬 허용 조건에서는 동일한 구동 결과를 산출해야 한다. 무구동 완료·보호 차단·복구 확인을 정상 구동과 구분해야 한다. |
...
| Door idempotency | 정상·허용된 새 동일 목표 요청은 무구동 DONE + ALREADY_AT_TARGET 및 새 완료음 없음; 동일 식별 재전달은 기존 결과만 반환 |
...
> **[검토 필요]** 보호 해제와 재동작의 실제 확인 방법·시간은 미정이다. 지정된 상태의 주기·나이는 §2.5의 잠정값을 따른다. 나머지 경로·실제 전달/나이 표현과 실측 검증은 남아 있다. 후보값·물리 구성과 확인 조건은 후속 설계에서 검토한다.
...
> **[검토 필요]** 최대 출력은 실제 방열 구성과 양방향 동작을 확인한 뒤 정한다. 후보 100 %는 실제 구성의 허용 출력 확정을 뜻하지 않는다.
...
> **[검토 필요]** 차단/복귀 80/60 °C는 후보값이다. 실제 소자·구동·방열 구성에 맞게 확인해야 한다.
...
| 상위 목표 유지 허용 | **[잠정]** 주기100 / 최대나이300 ms | 전달·목표 연결·출력 종료 확인 |
...
**[검토 필요]** 점멸 여부·패턴·GOODBYE 유지 시간은 별도로 정한다. 색상 지정만으로 이 항목이 확정된 것으로 처리하지 않는다.

구현 구성은 수정요청서의 S32K144EVB 내장 RGB LED·FTM0 3채널 PWM을 기준 후보로 둔다. **[검토 필요]** PTD15·PTD16·PTD0의 색상/채널 매핑, 공통 애노드·LOW 점등 조건과 출력 극성은 사용 보드 회로·설정으로 확인해야 한다. LOW 점등 구성이 확인되면 켜짐 비율에 맞춰 LOW 구간을 설정한다. 드라이버의 극성 반전 설정과 듀티 반전을 중복 적용하지 않는다. 이 구현 변환은 BCM 내부 책임이며 중앙이 원시 핀 듀티를 명령하지 않는다.
...
기본 동작은 §3.2.1·§3.2.5 및 §4.6을 따른다. §3.5의 잠정 피드백 구성 외에 아래 구현·검증이 필요하다.

- Fan·온도·조도 기반 출력 후보의 실측과 지원 구성
- 확정한 4종·색상의 실물 출력 검증, 실제 알림 패턴·유지 시간 결정 및 밝기 검증
- §3.2.5의 선행 5분/최대 5분과 실제 시각·허용 입력 구현/검증
...
- 선행 공조의 중앙 시각·실제 허용 입력 구현 및 잠정 5분/5분 확인
...
| 2 | 공통 초기화 실패·기한 초과, 공통 처리 불가 또는 활성 기능이 있으나 필요한 모든 제공 경로가 사용 불가 | `FAULT` |
| 3 | 공통 기반 정상이며 활성 기능 중 하나 이상이 현재 유효 결과를 하나 이상의 의도된 정보 사용 기능으로의 전달 경로로 제공 가능 | `ACTIVE` |
...
하나의 정보 사용 기능으로의 전달 경로만 끊기면 해당 경로 오류를 표시하고 나머지 경로의 제공은 유지한다. `FAULT` 자체가 모든 센싱·진단 처리를 중단하라는 명령은 아니다. 정상 기능이 남으면 개별 복구 실패에도 `ACTIVE`가 가능하다. 비활성 기능은 센서 고장으로 계수하지 않는다. 첫 데이터 기한이 지나면 대기를 무한 연장하지 않고 해당 기능의 확인 실패를 표시한다.
...
> **[검토 필요]** 후방 센서의 정상 미감지 구분 능력, 거리 후보값·전원 입력·최신성 기준을 확인해야 한다.
...
| <a id="cis-sys-fun-016"></a>CIS-SYS-FUN-016 | CIS는 후방 감지가 활성이고 현재 거리 정보가 유효하며 긴급 임계값보다 크고 주의 임계값 이하인 경우에만 `CAUTION`을 확정해야 한다. 경계와 평가 순서는 §5.6을 따라야 한다. |
| <a id="cis-sys-fun-017"></a>CIS-SYS-FUN-017 | CIS는 후방 감지가 활성이고 현재 거리 정보가 유효하며 긴급 임계값 이하인 경우 `EMERGENCY`를 확정해야 한다. 측정 가능 하한보다 작은 신뢰할 수 없는 값을 긴급 판정의 정상 거리로 사용하지 않아야 한다. |
| <a id="cis-sys-fun-018"></a>CIS-SYS-FUN-018 | CIS는 후방 감지가 활성이고 유효 거리로 주의 임계값 초과가 확인되거나 검증된 정상 미감지가 확인된 경우에만 `CLEAR`를 확정해야 한다. 초기 미수신·센서 무응답·범위 밖·비활성화는 위험 해제 근거로 사용하지 않아야 한다. |
| <a id="cis-sys-fun-019"></a>CIS-SYS-FUN-019 | CIS는 후방 정보의 유효성·최신성·활성 조건 또는 복구 확인이 충족되지 않으면 `CAUTION`/`EMERGENCY`/`CLEAR` 중 어느 것도 현재 유효 상태로 확정하지 않아야 한다. 확인 불가와 그 사유를 제공하고 과거 위험 상태를 현재 정상 상태로 재발행하지 않아야 한다. |
...
| <a id="cis-sys-fun-022"></a>CIS-SYS-FUN-022 | CIS는 유효한 탑승자 판정 결과 및 환경 측정값을 정의된 주기로 중앙 제어에 전송해야 한다. |
| <a id="cis-sys-fun-023"></a>CIS-SYS-FUN-023 | CIS는 제공하는 각 값과 후방 의미 상태에 유효 여부, 원본 시점/경과 시간, 갱신 구분 및 확인 불가 사유를 연결하여 제공해야 한다. 정상 값이 없는 경우에도 사용 가능한 경로로 품질·진단 정보를 제공해야 하며, 논리 의미와 인코딩은 구분해야 한다. |
...
| <a id="cis-sys-fun-026"></a>CIS-SYS-FUN-026 | CIS는 후방 위험 의미 판단을 담당하고 `CAUTION`/`EMERGENCY`/`CLEAR`를 VSS 의미 이름과 일치하는 상호 배타적 현재 상태로 제공해야 한다. 변경과 재동기화 시 §5.5.1의 품질·나이·순서 근거를 보존해야 하며 반복 전달을 새 위험 발생으로 만들지 않아야 한다. 후방 거리와 임계값은 내부 판단에만 사용하고 중앙·VSS 운영 인터페이스에 직접 제공하지 않아야 한다. |
...
다음은 논리 의미이며 특정 enum·비트 배치를 강제하지 않는다. 값의 `VALID`는 활성 조건, 새 입력의 품질·유효 범위, 최신성 및 필요한 복구 확인이 모두 충족된 경우에만 사용한다. 최초 미수신은 `NO_DATA`, 현재 입력의 검증 실패는 `INVALID`, 이전 유효 입력의 허용 나이 만료는 `STALE`로 구분한다. 비활성 및 활성 조건 확인 불가는 별도 서비스 사유로 제공한다. 어느 경우도 숫자 0, 탑승자 없음 또는 `CLEAR`로 대체하지 않는다.
...
| 온도·습도·조도 | 해당 값의 새 측정, 명시된 단위·측정 범위·센서 품질 및 나이 기준 충족 | 값별 무효. 공통 센서 고장이면 실제 영향 받는 값에 적용 |
| 후방 유효 거리 | 검증된 측정 범위 안의 새 거리이며 센서 품질·나이 충족 | 범위 밖·무응답·진단 실패는 유효 거리 아님 |
| 후방 정상 미감지 | 완료된 새 측정 회차에서 센서가 지원하는 정상 무물체 판정과 정상 진단이 함께 확인됨 | 단순 echo 없음·타임아웃만으로 정상 미감지를 확정하지 않음 |

원본 시점/경과 시간과 수신 시점/경과 시간는 [공통 품질 계약](#com-sys-qlt-001)을 따른다. 새 품질·진단 갱신을 받았다는 사실은 그 안의 센서 값이 정상이라는 뜻이 아니다. 구체 비전 품질·최대 인원수·센서 범위·나이 한도는 [미결 표](#cis-open-parameters)에 남겨 두며 임의 숫자로 정상 판정을 만들지 않는다.
...
이 문서에서는 실제 통신 신호나 숫자 Event ID를 정의하지 않는다.
CIS가 외부로 제공해야 하는 **의미 정보의 종류**만 정의한다.

#### 5.5.1 CIS → VSS (후방 근접 위험 현재 상태)

CIS는 아래 세 이름을 상호 배타적 현재 상태로 제공한다. 유효 상태가 바뀌면 새 판정으로 제공하고, 재접속·재동기화 때는 현재 상태와 품질을 다시 제공한다. 같은 판정의 반복 전달은 새 위험 발생이 아니며 과거 중간 상태를 순서대로 재생하지 않는다.

| Semantic Event | Meaning | Trigger |
|---|---|---|
| `REAR_OBSTACLE_CAUTION` | 유효한 현재 주의 상태 | 활성·유효 거리이고 긴급 임계값 < 거리 ≤ 주의 임계값 |
| `REAR_OBSTACLE_EMERGENCY` | 유효한 현재 긴급 상태 | 활성·유효 거리이고 거리 ≤ 긴급 임계값 |
| `REAR_OBSTACLE_CLEAR` | 유효한 현재 비위험 상태 | 활성·유효 거리 > 주의 임계값 또는 검증된 정상 미감지 |

`CLEAR`는 최초 정상 확인에서도 제공할 수 있다. 위험 이력이 없는 경우에도 현재 비위험이라는 뜻이며 새 경고 발생을 뜻하지 않는다. 비활성·확인 불가에서는 셋 중 하나를 유효한 현재 값으로 발행하지 않고 활성·품질·사유를 제공한다. 과거 상태를 진단용으로 보존해도 과거 시점과 신뢰할 수 없다는 품질 표시를 유지한다.

각 갱신에는 같은 관측 회차의 상태·활성·품질·원본 시점/경과 시간을 연결하고 새 판정/반복 전달 및 순서를 구분할 근거를 제공한다. 중앙 중계는 그 근거를 보존해야 한다. 정보 사용 기능은 오래된 상태로 최신 상태를 덮어쓰거나 중계 재수신으로 원본 정보 경과 시간을 초기화하지 않아야 한다. 재시작으로 순서 근거가 소실되면 정보 제공 기능의 새 실행 구간을 구분하고 새 유효 판정 전에는 확인 불가를 제공한다. 구체 counter·clock·프로토콜은 후속이다.

위험 값의 변화뿐 아니라 품질·활성 변경도 정보 사용 기능이 알 수 있게 제공한다. 상태의 주기·최신성은 §2.5의 잠정값을 적용하고, 전달·나이 표현은 인터페이스 설계에서 정한다. 상태가 같다는 이유로 정보 사용 기능이 최신성을 확인할 수 없게 해서는 안 된다. 완전 단절은 정보 사용 기능의 감시로 검출한다. VSS에서 입력을 신뢰할 수 없을 때 재생 유지/종료·대기는 §7.6을 따르며, 음향 종료를 CIS의 정상 위험 해제로 역변환하지 않는다.
...
| `CIS_STATE` | CIS 전체 상태(`STARTUP`/`READY`/`ACTIVE`/`FAULT`) | Diagnostic |
| 기능별 품질·활성·오류 | 값별 확인 불가/복구 중/현재 오류와 이력, 경로 상태 | Diagnostic / Validity |
| 원본 시점/경과 시간·갱신 구분 | 값과 같은 회차의 최신성·순서 근거 | Freshness |

> 실제 신호명, 숫자 값, 메시지 배치 및 전송 방식은 후속 인터페이스 설계에서 확정한다.

#### 5.5.3 후방 근접 위험 상태의 CIS 내부 처리 개념

아래 도식은 활성·유효 판정이 성립한 경로를 보인 것이다. 품질·활성 확인 불가 경로는 §5.4.1/§5.5.1을 따르며 셋 중 하나로 강제 분류하지 않는다.

`REAR_DISTANCE`는 CIS 내부 측정·판정 값이며 외부 운영 메시지·VSS 입력에 포함하지 않는다. 개발 중 내부 값 관찰과 요구된 외부 인터페이스는 구분한다. 외부 진단 전송을 별도 기능으로 추가하지 않는다.
...
판정 순서는 **활성 확인 → 품질·최신성·복구 확인 → 입력 종류 확인 → 거리 구간 분류**다. 유효한 입력만 아래 세 상태 중 하나로 분류한다. 기존 후보 10/40/70/100cm는 유지하되 구간의 양 끝을 겹치지 않게 정의했다. 수치를 실측 확정값으로 승격하지 않는다.

| 입력 조건 | 현재 결과 | 품질 |
|---|---|---|
| 유효 거리 `10 ≤ d ≤ 40 cm` | `REAR_OBSTACLE_EMERGENCY` | VALID |
| 유효 거리 `40 < d ≤ 70 cm` | `REAR_OBSTACLE_CAUTION` | VALID |
| 유효 거리 `70 < d ≤ 100 cm` | `REAR_OBSTACLE_CLEAR` | VALID |
| 검증된 정상 미감지 | `REAR_OBSTACLE_CLEAR` | VALID |
| `d < 10 cm`, `d > 100 cm`, 비정상 수치·센서 진단 실패 | 현재 위험 확인 불가 | INVALID |
| 측정 회차 무응답·초기 값 없음·원본 정보 경과 시간 만료 | 현재 위험 확인 불가 | 해당 NO_DATA/INVALID/STALE |
| 전원 불허·선택 후진 구성의 비후진 | 후방 감지 비활성, CLEAR 생성 없음 | 서비스 비활성 |
| 필수 활성 입력 확인 불가 | 후방 감지 활성 확인 불가, CLEAR 생성 없음 | 서비스 확인 불가 |

현재 후보의 측정 지원 범위는 `10~100cm`다. `101cm`처럼 범위 밖 숫자를 정상 미감지로 취급하지 않는다. 실제 센서가 검증된 무물체 상태를 구분할 수 없다면 ‘미감지→CLEAR’ 경로는 사용할 수 없고 해당 결과는 확인 불가다. 특히 10cm 미만의 근접 위험을 이 센서 후보만으로 검출한다고 보장하지 않는다. 지원 범위 확대는 센서 선정·검증 뒤 후보 표와 요구를 함께 갱신해야 한다.

| 항목 | Candidate / 현재 선택 |
|---|---|
| 내부 측정 거리 단위·범위 | `cm`, `[CANDIDATE] 10 ≤ d ≤ 100` — 외부 거리 전송 단위 아님 |
| 긴급 임계값 | `[CANDIDATE] 40 cm`, 경계 포함 |
| 주의 임계값 | `[CANDIDATE] 70 cm`, 긴급 미해당 및 경계 포함 |
| 해제 기준 | 유효 거리 `d > 70 cm` 또는 검증된 정상 미감지 |
| 경계 유지 정책 | 현재 후보는 분류 후 추가 히스테리시스 없음. 매 새 유효 판정에 같은 배타적 구간 적용 |

거리 필터는 분류 전 입력 품질의 일부이며 방식·노이즈 기준은 미정이다. 이후 히스테리시스나 지속 확인을 도입하면 진입/해제 경계·초기 상태·시간 및 확인 사례를 함께 바꿔야 한다. 이를 현재 문구에 숨겨서 적용하지 않는다.

<a id="sys-cis-s07"></a>

> **[검토 필요]** 후방 센서의 정상 미감지 구분 능력, 거리 후보값·전원 입력·최신성 기준을 확인해야 한다. 요청 결과 대기는 SysRS §2.3.1의 잠정값을 따른다. 실제 동작 완료·경고 전달 및 인지 기한은 검증이 필요하다.
...
| <a id="cis-sys-per-002"></a>CIS-SYS-PER-002 | CIS는 활성 탑승자/환경/근접 기능의 현재 결과와 품질을 정의된 내부 갱신 주기로 평가하여 논리 제공 경계에 준비해야 한다. 이는 실제 네트워크 송신 주기와 구분하며, 긴급 상태 준비는 PER-003의 기한을 만족해야 한다. | `[CANDIDATE] 200 ms` |
| <a id="cis-sys-per-003"></a>CIS-SYS-PER-003 | 후방 `EMERGENCY`를 CIS 내부에서 확정한 시점부터 그 상태와 품질을 CIS 논리 제공 경계에 준비하는 시점까지의 지연을 제한해야 한다. 센싱 이전 지연·네트워크 전송·VSS 중재 및 실제 음향 시작은 이 시간에 포함하지 않아야 한다. | `[CANDIDATE] ≤ 100 ms` |
...
| PER-003 | 유효 EMERGENCY 확정 → 논리 제공 준비 | 후보 100ms; 200ms 주기 대기 때문에 초과하지 않음 |
...
**[잠정]** 지정된 경로는 §2.5의 통신 P와 허용 지연 100ms를 적용한다. P=200ms 경로의 기준 시각이 0이면 예정 수신 기한 300/500/700ms의 세 번째 누락에서 통신 오류를 판단한다. 내부 갱신 후보 200ms에서 네트워크 600ms를 도출하지 않는다. 값별 최대 나이는 별도이므로 통신 오류 확정 전에 STALE일 수 있다. 주기 약속이 없는 이벤트 미발생은 누락으로 계수하지 않는다.
...
네트워크·VSS의 수용/중재/재생 시간을 합치지 않은 내부 후보다. 감지부터 실제 음향까지의 전체 기한은 VSS 중재·전달 시간을 포함하여 별도로 정한다.
...
| ID | 요구사항 |
|---|---|
| <a id="cis-sys-int-001"></a>CIS-SYS-INT-001 | CIS는 중앙을 통해 후방 감지 활성 판단에 필요한 차량 전원 허용 상태와 그 품질·최신성을 제공받아야 한다. 기본 구성은 유효한 전원 허용 동안 후방 감지를 활성화하며, 불허와 허용 확인 불가를 구분하고 어느 경우에도 정상 `CLEAR`로 치환하지 않아야 한다. |
...
비활성·확인 불가 전환 시 유효 위험 제공을 종료하고 사유를 제공한다. 이를 CLEAR 발행으로 대체하지 않는다. 재활성 후에는 그 이후의 새 입력으로 다시 판정한다. 이전 위험 상태를 자동 복원하지 않는다. 후진 연계 설정은 구성 값이며 모바일의 신규 사용자 제어 항목으로 추가하지 않는다. 이 후방 활성 조건을 탑승자·환경 센싱에 일괄 적용하지 않는다.
...
| <a id="cis-sys-int-005"></a>CIS-SYS-INT-005 | CIS는 후방 현재 위험 상태와 활성·품질·최신성 및 갱신 구분을 VSS가 확인할 수 있도록 §5.5.1에 따라 제공해야 한다. CIS는 음향을 직접 출력하거나 재생 우선순위를 결정하지 않아야 한다. |
...
| <a id="cis-sys-saf-004"></a>CIS-SYS-SAF-004 | CIS는 실내 영상 원본과 센서 원시 데이터를 어떠한 외부 인터페이스로도 노출하지 않아야 한다. 검증·가공된 탑승자 결과와 온도·습도·조도, 후방 위험 의미 및 품질은 정해진 논리 출력 범위로 제공해야 한다. |
...
| 환경 센서의 일시 측정 실패 | 영향 센서 응답·진단 회복 | 값별 범위·품질·나이를 통과한 새 측정 확인 | 해당 값만 신뢰할 수 없음; 공유 고장 영향은 함께 표시 |
| 후방 측정 실패 | 활성 조건 충족 및 센서 진단 회복 | 새 유효 거리 또는 검증된 정상 미감지의 확인; 최종 회차로 현 위험 재분류 | CLEAR 생성 없음, 과거 상태는 이력으로만 유지 |
...
| Exact boundaries | 10/40cm EMERGENCY, 40cm 직후/70cm CAUTION, 70cm 직후/100cm CLEAR; 한 회차에서 결과 하나 |
| Outside range | 9/101cm는 확인 불가, CLEAR나 유효 긴급으로 치환하지 않음 |
| No object | 정상 무물체 지원·진단이 검증된 회차만 CLEAR. echo 없음·센서 분리는 확인 불가 |
| Initial / stale | 첫 데이터 없음과 나이 만료를 정상 값으로 표시하지 않음 |
...
| Link isolation | VSS 경로만 단절이면 그 경로 오류 표시, 중앙 정상 제공 유지 |
...
| Reactivation | 비활성/신뢰할 수 없음 전환은 CLEAR 생성 없음; 재활성 이후 새 입력으로 재판정 |
| Resynchronization | CAUTION→EMERGENCY 중간 손실 뒤 최신 EMERGENCY+품질 재제공, 과거 CAUTION 반복으로 되돌리지 않음 |
...
| 시간 | Emergency internal-to-external delay | ≤ 100 ms |
...
| 후방 감지 | 내부 거리 단위 | cm (외부 거리 전송 없음) |
| 후방 감지 | Caution range | 유효 거리 40 < d ≤70cm |
| 후방 감지 | Emergency range | 유효 거리 10 ≤ d ≤40cm |
| 후방 감지 | Clear range | 유효 거리 70 < d ≤100cm 또는 검증된 정상 미감지 |
...
- CIS와 중앙 제어·VSS 간 물리 통신 경로 (Open Item: LPUART0/LPUART2 중 배정)
...
|---|---|---|
| 센서 모델·유효 범위·정상 무물체 구분 | CIS/HW: 사양과 단선/무물체/근거리 대조 실측 | 10cm 미만 및 범위 밖 감지 보장 없음. 정상 미감지 능력 없으면 해당 CLEAR 경로 사용 불가 |
| 영상 품질·계수 정확도·최대 인원수 | CIS: 준비된 입력·시야·조도별 인식 대조 | 존재/계수 VALID 및 복구 수용의 수치 확정 필요 |
| 환경 센서 범위·단위·나이 한도 | CIS/HW: 실제 부품·갱신 성능과 소비 기능 요구 대조 | 범위·최신성 검증 전 구현 판정 완료 아님 |
| 거리 필터·후보 임계값·복구 2회 | CIS: 경계·노이즈·간헐 오류 대조 | 기존 임계값과 신규 복구 횟수는 후보; 추가 히스테리시스는 현재 미적용 |
...
| 통신 P·첫 기한·허용 지연·나이·순서 표현 | **[잠정]** §2.5의 지정 상태 주기·나이 적용; 인터페이스+CIS+정보 사용 기능이 전달/시각 예산 확인 | 누락 3회와 품질 만료는 별도. 물리 전달·기한 충족 미검증 |
| VSS의 신뢰할 수 없는 입력·비활성 수용과 재생 정책 | CIS의 정보 제공과 VSS의 정보 사용 연결, §7.6.2 잠정 Hold | 후방 300ms, 끼임/잔류 1초 유지 뒤 중단·미확인. 나이·재생·통합 성능 미검증 |
...
> **[검토 필요]** 기능별 결과 대기는 §2.3.1의 잠정값을 따른다. 실제 출력 확인·전달 기한과 일부 실물 입력은 확인이 필요하다.
...
| ID | 요구사항 |
|---|---|
| <a id="mb-sys-req-001"></a>MB-SYS-REQ-001 | 모바일 애플리케이션은 통합 SR §7.3.1의 제어 대상에 대해서만 요청을 생성해야 한다. 자동 공조·선행 공조 사용 설정을 포함하고 창문·외부 조명 원격 제어를 추가해서는 안 된다. |
...
**[잠정]** 기본 시연은 앱 활성 화면에서 등록된 연결이 확인된 동안으로 제한한다. 백그라운드·앱 종료 중 응답 유지는 기본 범위에 포함하지 않는다. 차량 측 동작은 [§3.2.7](#central-digital-key)을 따른다.
...
차량이 제공한 신뢰성은 **차량 내부 기준**이며, 애플리케이션은 여기에 **무선 구간의 최신성**를 추가로 판정해야 한다.

차량이 `OK`로 보낸 값이라도 무선 구간에서 지연되면 사용자에게는 최신이 아니다.

| ID | 요구사항 |
|---|---|
| <a id="mb-sys-sem-007"></a>MB-SYS-SEM-007 | 수신 품질과 원본·무선 구간의 나이를 함께 평가해야 한다. INVALID·NO_DATA를 수신 직후라는 이유로 OK로 바꾸지 않고, 나이만 초과한 마지막 유효 값은 STALE로 구분해야 한다. |
...
> **[검토 필요]** 요청 결과 대기는 SysRS §2.3.1의 잠정값을 따른다. 실제 동작 완료·경고 전달 및 인지 기한은 검증이 필요하다. 실제 정보 전달 형식과 일부 입력 공급원은 미정이다.
...
| 실내 환경 | 자동 사용 설정·현재/목표 온도·현재 모드·Fan 설정/목표/측정·온도 출력·선행 설정/작업·오류 | 목표 온도·자동 사용·수동 순환·예정 시각·선행 사용/정지 |
...
| <a id="mb-sys-int-006"></a>MB-SYS-INT-006 | **[잠정]** 중앙에서 확인한 선행 공조 사용 설정·탑승 예정 시각·산출한 시작 시각과 예약/진행/종료/취소/실패 상태·사유를 제공받아야 한다. §3.2.5의 1회 예약·최대 5분 조건을 표시하고, 설정 요청 결과와 실제 선행 작업 상태를 구분해야 한다. |
...
| ID | 요구사항 |
|---|---|
| <a id="mb-sys-int-014"></a>MB-SYS-INT-014 | 통합 SR §7.3.1의 도어 목표, 목표 온도·자동 사용, 수동 순환, 예정 시각·선행 사용/정지, 조명 사용·밝기·색상, 근접 자동 해제 사용 여부 중 요청 대상과 값을 명확히 제공해야 한다. |
...
| ID | 요구사항 |
|---|---|
| <a id="mb-sys-nfr-004"></a>MB-SYS-NFR-004 | 동일한 값·품질·나이·연결 상태·요청 이력·사용자 확인 상태가 주어지면 동일한 표시를 산출해야 한다. 값만 같고 신뢰성이나 이력이 다른 경우를 같은 표시로 강제해서는 안 된다. |
...
| Digital key result | BCM의 실제 해제 확인 뒤 근접 원인 표시. 무구동 DONE·거부·UNKNOWN을 실제 해제로 표시하지 않음 |
...
| Supported controls | 자동 공조·선행 사용 설정과 조명 색상 포함. 창문·외부 조명 원격 제어 추가 없음 |
...
> **[검토 필요]** 후보값·물리 구성과 확인 조건은 후속 설계에서 검토한다. 요청 결과 대기는 SysRS §2.3.1의 잠정값을 따른다. 실제 동작 완료·경고 전달 및 인지 기한은 검증이 필요하다.
...
**[잠정]** §2.5의 MOBILE 일반 상태 제공 주기 500ms와 수신 후 경과 시간 한도 2초를 적용한다. 이는 프로젝트 표시 기본값이며 인지 실험에서 산출한 값은 아니다. 실제 주기·무선 지연을 확인해 조정한다. 원본 품질과 항목별 더 짧은 나이 한도를 함께 적용하고, 새 수신만으로 원본 정보 경과 시간을 초기화하지 않는다.
...
- §3.2.7의 실제 근접 판정·진입/이탈 기준·주기·유효 시간 및 기기 등록 해제 확인
...
- `STARTUP -> READY` 후에도 유효 지속형 경고 요청 또는 아직 의미 유효기간 안에 있는 단발성 음향 요청이 있으면 재중재 후 `PLAYING`으로 전이할 수 있다.
...
- 높은 우선순위 Session에 선점된 지속형 경고 요청은 유효 상태 자체를 잃지 않으며, 이후 다시 재생 대상으로 선택되면 재생 세션을 재활성화할 수 있다.
- 재생이 시작된 단발성 음향이 높은 우선순위 Session에 선점된 경우 자동 Resume하지 않는다.
...
> **[검토 필요]** 후방 센서의 정상 미감지 구분 능력, 거리 후보값·전원 입력·최신성 기준을 확인해야 한다. 요청 결과 대기는 SysRS §2.3.1의 잠정값을 따른다. 실제 동작 완료·경고 전달 및 인지 기한은 검증이 필요하다. 경고 입력과 발생·해제는 §3.2.3~3.2.4의 잠정 조건을 따른다. 실제 장치·입력 신뢰성·전달 기한은 확인이 필요하다.
...
| <a id="vss-sys-fun-010"></a>VSS-SYS-FUN-010 | 이미 재생을 시작한 단발성 음향은 다른 요청의 선점이나 출력 고장으로 중단되면 자동 재개하지 않아야 한다. 이 규칙은 일반 피드백과 도어 잠금 실패 경고에 모두 적용한다. |
...
| <a id="vss-sys-fun-012"></a>VSS-SYS-FUN-012 | 지속형 경고는 §7.6에 따른 Effective State가 활성인 동안 중재 후보로 유지하고, 출력 가능한 재생 대상으로 선택된 동안 해당 반복 또는 지속 음향을 제공해야 한다. |
...
| <a id="vss-sys-fun-030"></a>VSS-SYS-FUN-030 | 유효한 실제 잠금 완료 Event에 잠금 완료 단발성 음향을 생성하고 FUN-011을 적용해야 한다. 무구동 동일 목표 완료에는 새 음향을 생성하지 않아야 한다. |
| <a id="vss-sys-fun-031"></a>VSS-SYS-FUN-031 | 유효한 실제 잠금 해제 완료 Event에 잠금 완료와 구분 가능한 단발성 음향을 생성하고 FUN-011을 적용해야 한다. 무구동 동일 목표 완료에는 새 음향을 생성하지 않아야 한다. |
...
#### 7.4.2 지속 상태·재중재 요구사항

| ID | 요구사항 |
|---|---|
| <a id="vss-sys-fun-035"></a>VSS-SYS-FUN-035 | 각 지속형 입력의 마지막 유효 의미 상태, 현재 품질·기능 가용 상태와 중재에 쓰는 Effective State를 구분하고 §7.6의 신뢰할 수 없는 입력·비활성·복구 규칙을 적용해야 한다. |
| <a id="vss-sys-fun-036"></a>VSS-SYS-FUN-036 | 지속형 입력의 활성·해제·등급 변경, 현재 Playback Session의 종료, 선점 조건 변경 또는 오류 복구가 발생한 경우 VSS ECU는 현재 유효 요청 집합을 다시 중재해야 한다. |
| <a id="vss-sys-fun-037"></a>VSS-SYS-FUN-037 | 높은 우선순위 Playback Session에 의해 선점된 지속형 주의·긴급 경고의 Effective State가 계속 활성인 경우, 재중재 결과 해당 요청이 다시 재생 대상으로 선택되면 VSS ECU는 해당 Playback Session을 다시 활성화해야 한다. |
...
| <a id="vss-sys-fun-039"></a>VSS-SYS-FUN-039 | **[잠정]** 단발성 음향의 Max Age는 원 발생 시점부터 2초로 계산해야 한다. 실제 음향 시작은 나이 2초 미만일 때만 허용하며 송신 대기·중계·VSS 대기·출력 준비를 포함해야 한다. 재전송·복구·수용 시점에 나이를 다시 시작해서는 안 된다. 발생 나이 또는 남은 유효기간을 확인할 수 없는 요청은 재생하지 않아야 한다. |
| <a id="vss-sys-fun-040"></a>VSS-SYS-FUN-040 | **[잠정]** 아직 시작되지 않은 단발성 음향은 STARTUP·선점·출력 제한 중에도 원 발생 기준 Max Age 미만에서만 대기할 수 있으며, 실제 재생 시작 직전에 유효기간을 다시 확인해야 한다. Max Age 도달 시 미시작 요청을 폐기하고 이미 시작한 음향은 §7.6.3의 종료 규칙을 따라야 한다. |
| <a id="vss-sys-fun-041"></a>VSS-SYS-FUN-041 | 단발성 이벤트의 음향 재생을 시작한 뒤 선점된 경우에는 VSS-SYS-FUN-010을 적용하고, 아직 음향 재생을 시작하지 않은 단발성 이벤트에는 VSS-SYS-FUN-039 및 VSS-SYS-FUN-040을 적용해야 한다. |
| <a id="vss-sys-fun-042"></a>VSS-SYS-FUN-042 | 후방 Effective State가 EMERGENCY에서 CAUTION으로 변경되면 기존 후방 긴급 후보를 주의 후보로 대체하고 재중재해야 한다. 이전 긴급 출력 종료는 PER-010을 따르며, 주의 출력은 새로 선택된 재생 대상인 경우에만 시작해야 한다. |
...
| `DOOR_LOCK_COMPLETE` | 요청 실행에 의한 실제 잠금 전이 확인 (§2.2) | Feedback | 단발성 피드백 음향 |
| `DOOR_UNLOCK_COMPLETE` | 요청 실행에 의한 실제 잠금 해제 전이 확인 (§2.2) | Feedback | Lock feedback과 구분 가능한 feedback |
| `DOOR_LOCK_ERROR` | 수용 후 실제 LOCK 실행의 목표 확인 실패 (§2.2) | Warning | Lock 완료와 구분 가능한 warning |
...
후방 상태를 받은 VSS는 해당 후보를 갱신한 뒤 전체 후보를 재중재한다. `CAUTION/EMERGENCY` 수용만으로 다른 선택된 재생 대상을 무시하고 후방 음향을 출력하지 않는다. `CLEAR`는 후방 후보만 제거한다. [기본 재생 규칙](#vss-draft-behavior)을 적용한다.
...
| 주의 내부 | 후방 접근 → 잠금 실패 |
| 일반 피드백 내부 | 잠금 해제 완료 → 잠금 완료 → 사용 종료 → 사용 시작 |

당장 진행 중인 끼임·충돌 경고를 먼저 제공하는 초안이다. 같은 의미의 여러 단발성 음향은 원 발생 순서, 동률이면 발생 식별자의 고정 순서로 정한다. 새 후보가 표에서 더 앞서면 현재 음향을 선점한다. 수신 순서나 현재 재생 중이라는 이유로 순서를 뒤집지 않는다. 위험 유형 간 순서와 장시간 대기로 다른 위험을 알리지 못하는 경우는 통합 시연에서 다시 검토한다.

| 상황 | 기본 동작 |
|---|---|
| 지속형 경고가 다른 경고에 선점됨 | 최신 의미와 품질을 유지. 다시 선택될 때만 재생 |
| 아직 시작하지 않은 단발성 음향 | [잠정] 원 발생 기준 2초 미만에서만 대기. 실제 시작 직전 재확인 |
| 이미 시작한 단발성 음향이 선점·고장으로 중단됨 | 종료 처리. 이후 자동 재개·재전달 재생 금지 |
| 같은 발생의 재전송 | 새 요청·새 유효기간으로 만들지 않음 |
| 특정 경고 CLEAR | 그 후보만 제거하고 재중재. 다른 경고까지 끄지 않음 |
| 후방 긴급 → 주의 | 후방 긴급 출력 종료 후 주의 후보로 변경. 다른 긴급이 있으면 그것을 재생 |
| 일부 자산 고장 | 해당 의미는 출력 불가로 기록. 정상 자산의 후보는 계속 처리 |
| 공통 출력 경로 고장 | 재생 중단, 복구 중에도 FAULT/UNAVAILABLE. 성공 뒤 최신 후보 재평가 |

입력 수용은 재생 완료가 아니다. 높은 순위가 계속 활성인 동안 낮은 순위의 출력 시작은 보장하지 않는다. 전체 경고의 인지 기한까지 닫힌 것은 아니며, 수용→중재 및 차량 발생→인지 기한은 후속 대조 대상이다. 단발성 이벤트 발생 후 경과 시간과 중복 여부를 확인하는 실제 전달 방식은 인터페이스 단계에서 정한다.
...
Hold는 정상 경고의 새 확인이 아니다. 입력을 처음 신뢰할 수 없게 된 시점부터 시간을 세며, 같은 상태의 입력 반복·재전송으로 유지 시간을 연장하지 않는다. 같은 입력에서 여러 사유로 신뢰할 수 없는 상태가 이어지면 하나의 연속된 구간으로 본다. 유효 DISABLED는 수신 품질과 출처가 정상일 때만 적용한다. 입력을 신뢰할 수 없어 중단된 음향은 CLEAR나 위험 해제를 뜻하지 않는다.
...
Hold는 해당 입력을 처음 신뢰할 수 없게 된 시점부터 단조 증가 시간으로 측정한다. 수신 중단에 의한 STALE은 원본 정보 경과 시간 한도 도달 시점부터 세며, 그보다 먼저 INVALID/SNA가 확인되면 그 시점부터 센다. 늦은 평가나 입력의 신뢰성 상실을 알리는 메시지 수신으로 시작을 뒤로 미루지 않는다. Hold에 도달하면 마지막 경고 후보를 제거하고 해당 출력의 중단을 지시하며 다른 유효 후보를 다시 중재한다. 그 전에 새 유효 상태가 복구되면 §7.6.1로 재평가한다. 같은 시점에는 새 유효 입력을 먼저 반영한다. 신뢰할 수 없는 입력의 반복·재전달·다른 경고 선점은 Hold를 다시 시작하지 않는다.

유지 중에도 현재 입력을 신뢰할 수 없다는 품질 표시를 유지한다. **[잠정]** Hold 만료로 종료가 필요한 음향은 만료 시점부터 100ms 이내에 해당 출력을 끝낸다(VSS-SYS-PER-006). 평가 지연으로 기한을 다시 시작하지 않으며 출력 종료 실패는 기존 출력 진단으로 제공한다. 이 시간은 로컬 끼임 보호·차단의 지연 허용값이 아니다. 음향 중단 이후에도 중앙·MOBILE의 미확인 및 미해제 경고 이력은 유지한다. 입력 미확인·유지/중단 상태는 진단·시험에서 확인하며, 별도 고장 음향은 추가하지 않는다. 끼임 반전 완료나 STOP만으로 중앙의 CLEAR를 만들어서는 안 된다. 실제 경고 전달·인지 성능과 잠정 시간의 적정성은 확인이 필요하다.
...
**[잠정]** §7.5.1의 모든 단발성 음향은 원 발생부터 실제 음향 시작까지의 나이가 2초 미만일 때만 재생을 시작한다. 정확히 2초에 도달했거나 나이·동일 발생 여부를 확인할 수 없으면 새로 재생하지 않는다. 수신·중계·기동·중재 대기와 출력 시작 준비 시간을 모두 포함하며, 재전달로 나이를 다시 시작하지 않는다.

재생 대상 선택만으로 유효기간을 확정하지 않고 실제 시작 직전에도 다시 확인한다. 유효기간 안에 시작한 음향은 자산의 재생 길이와 기존 선점·고장 규칙에 따라 끝낸다. 따라서 원 발생 후 2초가 됐다는 이유만으로 이미 정상 시작한 음향을 자르지 않는다. 이 2초는 기존 음향 길이 후보 ≤2초와 별개이며 모든 요청의 재생을 보장하는 시간도 아니다. 선점·고장으로 중단된 단발성 음향은 자동 재개하지 않는다.

<a id="sys-vss-s07"></a>

> **[검토 필요]** 요청 결과 대기는 SysRS §2.3.1의 잠정값을 따른다. 실제 동작 완료·경고 전달 및 인지 기한은 검증이 필요하다. 후방 센서의 정상 미감지 구분 능력, 거리 후보값·전원 입력·최신성 기준을 확인해야 한다.
...
| <a id="vss-sys-per-005"></a>VSS-SYS-PER-005 | 새 긴급 경고의 유효 수용 즉시 재중재한 결과 그 요청이 재생 대상으로 선택되었고 출력 가능하면, 수용부터 낮은 우선순위 음향을 종료하고 해당 긴급 출력을 시작하기까지의 시간을 제한해야 한다. | `≤ 50 ms` |
| <a id="vss-sys-per-006"></a>VSS-SYS-PER-006 | **[잠정]** 상태 유지형 경고의 유효한 CLEAR/해당 기능 DISABLED 수용 또는 §7.6.2의 Hold 만료에 따라 출력 종료가 필요한 경우, 수용 시점 또는 Hold 기한부터 해당 경고의 실제 음향 출력 종료까지의 지연을 제한해야 한다. 다른 유효 경고는 재중재하고 음향 중단을 위험 해제로 바꾸어서는 안 된다. | `≤ 100 ms` |
| <a id="vss-sys-per-007"></a>VSS-SYS-PER-007 | 후방 CAUTION에서 EMERGENCY로의 유효 변경을 수용해 즉시 재중재한 결과 후방 긴급이 재생 대상으로 선택되었고 출력 가능하면, 수용부터 해당 긴급 출력 시작까지의 시간을 제한해야 한다. | `[CANDIDATE] ≤ 50 ms` |
...
> **[검토 필요]** 후방 센서의 정상 미감지 구분 능력, 거리 후보값·전원 입력·최신성 기준을 확인해야 한다. 요청 결과 대기는 SysRS §2.3.1의 잠정값을 따른다. 실제 동작 완료·경고 전달 및 인지 기한은 검증이 필요하다. 실제 정보 전달 형식과 일부 입력 공급원은 미정이다.
...
| <a id="vss-sys-int-012"></a>VSS-SYS-INT-012 | **[잠정]** Startup/Wake 또는 통신 복구 이전에 발생한 미재생 단발성 이벤트의 나이가 원 발생 기준 2초에 도달한 경우 해당 Event를 신규 유효 Event처럼 재생 또는 재전달해서는 안 된다. |
...
> **[검토 필요]** 요청 결과 대기는 SysRS §2.3.1의 잠정값을 따른다. 실제 동작 완료·경고 전달 및 인지 기한은 검증이 필요하다. 상태·활성 목표의 감시는 SysRS §2.5~2.6의 잠정안을 따른다. 실물 입력·복구·차단 확인과 나머지 경로는 확인이 필요하다.
...
| 지속형 경고 재중재 | Rear=CAUTION 중 Anti-Pinch=ACTIVE, 이후 Anti-Pinch=CLEAR이고 Rear가 계속 CAUTION이면 Rear Caution Session이 다시 활성화됨 |
| 선점된 지속형 입력의 갱신 | 높은 우선순위 Session 중 다른 지속형 입력이 변경되어도 해당 최신 Effective State가 보존되고 이후 재중재에 사용됨 |
...
| Blocked one-shot expiry | 전달과 VSS 대기를 합한 원 발생 나이가 Max Age에 도달하면 폐기. 재전송으로 연장하지 않음 |
| Preempted one-shot | 재생을 시작한 Feedback 또는 잠금 실패 단발성 음향이 선점·고장으로 중단되면 자동 재개하지 않음 |
| Repeated state input | 동일 ACTIVE 상태가 주기적으로 반복 수신되어도 재생 세션이 매 프레임마다 처음부터 재시작되지 않음 |
| Startup event delivery | 미재생 Event의 원 발생 나이가 Max Age 미만일 때 전달·중재. Wake/복구 자체로 새 사용 시작 Event를 만들지 않음 |
...
| Recovery resynchronization | 최신 유효 의미/품질/가용 상태로 재중재. CLEAR는 해당 후보만 제거, 과거 이력 재생 금지 |
| Idle fault transition | READY 중 Output-critical Fault 발생 시 FAULT 전이 |
| Degraded fault | 불가 자산의 후보를 제외하고 정상 자산 후보를 중재. 재생 후보가 없으면 READY/DEGRADED |
...
`60/80/100%`는 내부 상대 출력 수준이며 음압(dBA)이나 외부 SET_VOLUME 명령의 요구가 아니다. 잠금 해제의 2회 음향·150–300ms 간격도 검증 전 후보다. 중복 억제 150ms는 보조값이며 동일 발생 식별을 대신하지 않는다. 실제 발생 식별과 카운터 폭은 미정이다.
...
> **[검토 필요]** 실제 정보 전달 형식과 일부 입력 공급원은 미정이다. 요청 결과 대기는 SysRS §2.3.1의 잠정값을 따른다. 실제 동작 완료·경고 전달 및 인지 기한은 검증이 필요하다.
...
- 후방 위험 판단 담당은 CIS이다. VSS 소비 규칙은 §7.6을 따른다. 센서 검증·후보 수치·물리 전달 경로는 미정

#### 7.14.2 인터페이스·네트워크 설계 시 결정

- VSS와 상대 ECU 간 물리 통신 경로 및 실제 Protocol
- 각 의미 정보의 Source/Network Producer/Consumer
- §7.6.3의 잠정 Max Age 2초·실제 시작 시점·발생 나이 전달 검증
...
- 지속형 입력별 SNA 조건
- §2.5·§7.6.2의 잠정 Freshness/Hold 충족과 실제 전달·나이 표현 확인
...
- 잠금 완료/실패 및 사용 시작/종료 Event의 발생 나이, 중복 이력과 Wake 전원 유지
...
지속형 위험 정보는 정상 `CLEAR`와 통신 미수신/STALE/INVALID/SNA를 구분해야 한다. 높은 우선순위에 선점된 지속형 주의 경고는 의미 상태가 계속 유효하면 재중재 후 다시 활성화될 수 있으나, 선점된 단발성 음향은 자동 Resume하지 않는다.
...
> **[검토 필요]** 외부 자동 점등은 §9.6.2.1, ON/OFF·10초 임시 점등·피드백 미지원은 §3.5의 잠정안을 따른다. 실제 센서·부품·보호·배치와 기한 충족은 확인이 필요하다.
...
> **[검토 필요]** 원터치 입력 구분·자동 환기는 §3.5·§3.2.6의 잠정안을 따른다. 실제 스위치·위치 보정·반전 목표/시간은 미정이다. 끼임 경고는 §3.2.4의 잠정 현장 확인 조건으로 해제한다. 확인 입력·정지 피드백의 실제 구성은 미정이며 반전 완료·STOP만으로 해제하지 않는다.
...
> **[검토 필요]** 원터치 입력 구분·자동 환기는 §3.5·§3.2.6의 잠정안을 따른다. 실제 스위치·위치 보정·반전 목표/시간은 미정이다.
...
| <a id="win-sys-per-009"></a>`WIN-SYS-PER-009` | ECU는 주기 정보의 약속된 수신 기한과 값의 허용 나이를 감시해야 한다. 이동 명령은 해당 요청의 유효기간을 확인하고, 새 명령이 발생하지 않는 정상 대기를 통신 고장으로 판단해서는 안 된다. | 시간값은 네트워크 설계에서 확정 |

<a id="sys-window-s08"></a>

> **[검토 필요]** 끼임 경고는 §3.2.4의 잠정 현장 확인 조건으로 해제한다. 확인 입력·정지 피드백의 실제 구성은 미정이며 반전 완료·STOP만으로 해제하지 않는다.
...
| 유효 요청의 목표가 이미 충족됨 | 허용·피드백 확인 후 무구동 DONE |
...
| `WIN_COMM_MAX_AGE` | **[잠정]** 300 ms | §2.6 상위 이동 목표 유지 허용의 원본 최대 나이. 로컬 이동/반전과 별도 | 주기100ms·전달 지연·상실 정지 확인 |
...
- §3.2.6의 환기 목표 80%·취소·종료 조건의 실기 확인
- 전원 상태별 창문 허용 정책
- 끼임 이후 사용자 재시도 정책
...
> **[검토 필요]** 원터치 입력 구분·자동 환기는 §3.5·§3.2.6의 잠정안을 따른다. 실제 스위치·위치 보정·반전 목표/시간은 미정이다. 외부 자동 점등은 §9.6.2.1, ON/OFF·10초 임시 점등·피드백 미지원은 §3.5의 잠정안을 따른다. 실제 센서·부품·보호·배치와 기한 충족은 확인이 필요하다.
...
**[잠정]** 중앙은 유효한 차량 사용 중 상태, 현장 자동 점등 사용 허용, 실제 출력 허용과 정상 로컬 보호가 함께 확인될 때 자동 목표를 계산한다. 외부 조도는 §3.2.3의 별도 입력을 lux로 제공하며 §2.5의 품질·나이 기준을 따른다. 초기·재기동에는 자동 사용이 해제되어 있고 현장에서 새로 허용해야 한다. MOBILE에 새 조명 제어를 추가하지 않는다.
...
지속 시간은 중앙이 조건을 만족하는 첫 새 유효 관측을 받은 때부터 단조 증가 시간으로 센다. 기준 구간을 벗어나거나 중앙의 새 관측 수신 간격이 600ms를 넘으면 진행 중 확인 시간을 초기화한다. 같은 관측의 재전달만으로 시간을 채우지 않고, 설정 시간 이상 경과한 뒤 도착한 새 유효 관측에서도 조건이 맞을 때 전환한다. 새 관측의 시점·나이가 확인되지 않으면 조건 성립으로 보지 않는다.
...
| 안전/수동 목표가 선점 | 임시 작업 중단. 선점 종료 후 과거 임시 작업 자동 재개 금지 |
...
| <a id="els-sys-per-005"></a>`ELS-SYS-PER-005` | 주기 제공이 약속된 상태·허용 정보 또는 연결 진단의 기한과 나이를 감시해야 한다. 이벤트 명령 미발생을 통신 상실로 판단해서는 안 되며, 임시 점등의 정상 유지 기한 종료도 통신 고장과 구분해야 한다. | 시간값은 네트워크 설계에서 확정 |
...
| `T_ELS_TEMPORARY_MAX` | **[잠정]** 10 s | 최초 수용부터 측정, 중복 수신으로 연장하지 않음 | 실제 출력 적용·종료·선점 확인 |
...
| `ELS_COMM_MAX_AGE` | **[잠정]** 300 ms | §2.6 상위 출력 목표 유지 허용의 원본 최대 나이. 조도 나이·임시10초와 별도 | 주기100ms·상실 OFF·갱신/종료 확인 |
...
- §3.2.3의 별도 외부 조도 입력·§2.5의 잠정 품질/나이 기준과 실물 센서 연결
...
이 표는 정보의 의미를 정해 제공하는 기능과 이를 사용하는 기능을 나타낸다. 화살표나 정보 사용 기능 표기는 실제 CAN/UART 직접 연결·패킷 경로 확정이 아니다. 중앙이 전달하더라도 원본 의미 소유자가 자동으로 중앙으로 바뀌지 않는다. 후보/TBD는 합의 완료가 아니다.

| 정보 | 생산/확정 책임 | 소비 책임 | 전달 의미 | 발생·종료·신뢰성 상실 조건 |
|---|---|---|---|---|
| 도어 잠금/개폐/종합 상태 | BCM | 중앙·MOBILE | 독립 상태 2축+종합 신뢰성 | 목표 변경/실측 갱신; 상태 정보를 신뢰할 수 없을 때 UNKNOWN/유효성 |
| 도어 처리 결과 | BCM, 중앙의 차량 요청 결과는 별도 | 중앙→MOBILE | 요청 식별·결과·사유 | 무구동 DONE, 거부, 실패 구분 |
| 도어 완료/이상 발생 | 중앙 도어 의미 판단 기능 | VSS | 단발성 이벤트의 발생 식별·경과 시간 | §2.2 실제 전이/실행 실패 기준 적용; 무구동·거부·UNKNOWN은 새 발생 아님 |
| 실내 온도·습도·조도 | CIS | 중앙·MOBILE(전달 경로 별도) | 측정값+원본 유효성·나이 근거 | 원본 무갱신을 중계 수신으로 새 값 처리 금지 |
| 목표 온도·자동 공조 설정 | MOBILE 요청, 확정 설정 소유 중앙 | 중앙·MOBILE | 요청 설정/확정 설정 | 수락/적용/정책 override 구분 |
| Fan/냉난방 실행 명령 | 중앙 | BCM | 확정 수준·방향+유효성 | 공유 자원 중재; 보호는 BCM 우선 |
| 선행 공조 | 중앙 시각·예약·공조 판단 | BCM 실행·MOBILE 표시 | 사용/예정·시작 시각·작업 상태·정지 결과 | **[잠정]** §3.2.5의 5분 전·최대 5분. 설정과 실행 결과 구분 |
...
| 창문 확정 요청 | 중앙 / 로컬은 WINDOW 입력 | WINDOW | 방향·STOP·목표·유효성·식별 | 상위 권한과 로컬 보호 분리. 자동 환기는 §3.2.6의 잠정 80% 목표·소유한 이동만 정지 |
| 창문 상태/위치/결과 | WINDOW | 중앙·MOBILE | 0%=열림,100%=닫힘; 유효성 | 앱 현행은 표시, 제어 자동 추가 금지 |
| 끼임 발생 | WINDOW | 중앙/VSS(전달 계약 미정) | 한 번의 occurrence, 재전달 동일 ID | 발생 이벤트는 해제 상태를 대신하지 않음 |
...
| 후방 위험 현재 상태 | CIS | VSS | CLEAR/CAUTION/EMERGENCY+품질 | 감지 불가≠CLEAR, 현재 상태 재동기화 |
...
| 차량 사용 시작/종료 | 중앙 (차량 상태 입력 근거) | VSS 및 선택 조명 | 별개의 단발성 이벤트/임시 명령 | **[잠정]** 유효 사용 상태 전이. 첫 수신은 발생 아님. 음향 시작 나이2초 미만 잠정 적용. 실제 전달·전원 유지 미검증 |
...
| 근접 자동 해제 | **[잠정]** ESP32 등록·근접 → 중앙 설정·허용 판단 | BCM 실행·MOBILE 표시 | 사용 설정·현재 사용 가능 여부·확정 해제 원인 | §3.2.7. 기존 VSS 도어 완료음 연결; 별도 음향·조명 알림 종류 추가 없음 |
...
> **[검토 필요]** 도어는 독립 접점의 잠정 구성을 따른다. 실제 센서·상태 확인 방법·결과 확인 기한·완료음 전달 기한은 검증이 필요하다. 선행 공조는 §3.2.5의 잠정 조건을 따른다. 실제 시각·허용 입력과 방열 피드백 검증은 필요하다. 색상 기준은 §4.13.6를 따른다. 패턴·유지 시간과 밝기의 실제 표현·검증 조건은 미정이다.
...
자동/선행 공조에서 `d = |목표 온도 − 실내 온도|`를 사용한다. 수동 순환은 사용자가 선택한 수준을 사용한다.
...
BCM은 다음 조건에 따라 거부 또는 무구동 완료를 결정한다. **상위 판단 기능은 명령을 생성하기 전에 필요한 상태·허용 조건을 확인해야 하며, BCM의 구동 직전 로컬 확인을 대신하지 않는다.**

| 조건 | BCM 동작 |
|---|---|
| 물리 개폐 상태가 `CLOSED` 아님 (잠금 시) | 거부 · `DOOR_OPEN` |
| 도어 상태가 `UNTRUSTED` | 거부 · `STATE_UNTRUSTED` |
| 정상·허용된 새 요청의 목표 상태가 현재와 동일 | 구동 없이 `DONE` + `ALREADY_AT_TARGET`; 새 완료 발생 없음 (§2.2) |
...
| 필요 정보 | 용도 | 정보 사용 기능 | 전송 방식 |
|---|---|---|---|
| 탑승자 존재 여부 | 실내 상태 파악 | Central Controller | 미정(TBD) |
| 탑승자 인원수 | 실내 상태 파악 | Central Controller | 미정(TBD) |
| 실내 온도 | 공조 등 상위 기능 활용 | Central Controller | 미정(TBD) |
| 실내 습도 | 공조 등 상위 기능 활용 | Central Controller | 미정(TBD) |
| 조도 | 조명 등 상위 기능 활용 | Central Controller | 미정(TBD) |
| 후방 물체 주의 상태 | 후방 주의 경고 시작 | VSS | 미정(TBD) |
| 후방 물체 긴급 상태 | 후방 긴급 경고로 전환 | VSS | 미정(TBD) |
| 후방 위험 해제 상태 | 유효한 비위험 상태 확인; 비활성/신뢰할 수 없는 상태는 별도 | VSS | 미정(TBD) |
| 각 정보의 활성·유효 여부·원본 정보 경과 시간·갱신 구분 | 정상·신뢰할 수 없음·비활성 및 재동기화 구분 | Central Controller / VSS | 미정(TBD) |
| 기능/경로별 현재 오류·복구·이력 | 부분 사용 가능 여부 구분 | Central Controller / VSS(관련 후방 품질) | 미정(TBD) |
| CIS 상태(`STARTUP`/`READY`/`ACTIVE`/`FAULT`) | 상위 진단 | Central Controller | 미정(TBD) |
...
- 초음파 원시 거리와 임계값은 외부 운영 출력에서 제외한다. CIS가 판정한다.
- 거리 임계값 및 위험도 판정은 CIS에서 정의한다.
- VSS는 `주의 / 긴급 / 해제` 현재 상태와 활성·품질·나이 근거를 소비한다. [§5.5.1](#sys-cis-s05)의 동일 판정·순서·복구 계약을 적용하며 신뢰할 수 없는 상태를 CLEAR로 변환하지 않는다.
...
**[잠정]** 등록·근접 정보의 차량 측 담당은 ESP32이며 중앙과의 책임 분담은 §3.2.7을 따른다. 게이트웨이의 나머지 상세 구성은 후속 설계에서 정하고 MOBILE은 게이트웨이 너머의 개별 실행 노드와 직접 통신하는 구조를 전제하지 않는다.
...
| 대상 차량·세션·요청 식별 및 원 요청-명령 연계 | 종단 중복·순서 판별; COM-SYS-REQ-002 | 중앙 판단 기능 | 요청·결과 의미는 공통 계약을 따름. 전달 형식은 미정 |
| 기기 등록·연결 확인 응답 | 등록된 연결 확인 | **[잠정]** ESP32 → 중앙 | 무선 계층 절차 사용; 연결 식별·전달 형식은 미정 |
| 근접 자동 해제 사용 설정 | 등록된 단말·차량의 자동 해제 사용/해제 | 중앙 판단 기능 | §3.2.7; 요청·설정 반영 형식은 미정 |
| 디지털 키 무선 응답 | 차량 측 단말 식별·근접 판단의 입력 | ESP32 | 무선 연결 계층 절차; 앱의 자동 해제 명령은 아님 |
| 날짜·시간대 포함 탑승 예정 시각 | [잠정] 중앙 시각으로 예정 5분 전 시작 산출 | 중앙 판단 기능 | 미정(TBD) |
...
> **[검토 필요]** 색상 기준은 §4.13.6를 따른다. 패턴·유지 시간과 밝기의 실제 표현·검증 조건은 미정이다. 실제 정보 전달 형식과 일부 입력 공급원은 미정이다.
...
| 필요 정보 | 용도 | 논리 담당 | 전송 방식 |
|---|---|---|---|
| 근접 자동 해제의 반영 설정·사용 가능 여부·해제 원인 | 설정과 실제 도어 해제 결과를 구분 표시 | 중앙(BCM 결과 연결) | §3.2.7; 전달 형식은 미정 |
| 설정된 목표 온도 · 자동 공조 사용 여부 | 설정 표시 | 중앙 판단 기능 | 미정(TBD) |
| 선행 공조 사용·예정/시작 시각·작업 상태/사유 | [잠정] 1회 예약·최대 5분 표시, 설정과 실행/종료 구분 | 중앙 판단 기능 | 미정(TBD) |
...
| 방향 | 정보 | 담당 |
|---|---|---|
| 중앙 제어 → VSS | 차량 사용·도어 이벤트, 끼임·잔류 탑승자 경고 | 중앙이 의미 판단. 도어는 BCM 실행, 끼임은 WINDOW, 탑승자는 CIS 관측을 근거로 사용 |
| CIS → VSS | 후방 위험의 현재 의미·품질·가용 상태 | CIS가 의미 판단. 중앙 중계 여부와 실제 전달 노드는 미정 |
| VSS → 외부 | 동작 상태·입력 수용 가능 여부·서비스 가용성·고장 | VSS 생산. 가용성·현재 고장·고장 이력은 중앙을 통해 MOBILE에 제공 |

의미 판단 담당과 물리 전달 노드는 구분한다. 중앙이 후방 정보를 중계해도 CIS의 의미를 다시 판정하지 않는다. 차량 사용·중앙 경고의 조건은 [§3.2.3](#central-input-profile)·[§3.2.4](#central-warning-rules)을 따른다.
...
| `DOOR_LOCK_COMPLETE` | 단발성 이벤트: 요청 실행에 의한 실제 잠금 전이 확인 | 일반 피드백 | 중앙, BCM 실행 근거 |
| `DOOR_UNLOCK_COMPLETE` | 단발성 이벤트: 요청 실행에 의한 실제 잠금 해제 전이 확인 | 일반 피드백 | 중앙, BCM 실행 근거 |
...
| `REAR_OBSTACLE_STATE` | 지속 상태: `CLEAR / CAUTION / EMERGENCY` | 주의 / 긴급 | CIS |

도어 무구동 완료에서는 새 이벤트를 생성하지 않는다. 도어 이벤트의 발생·중복 구분은 [§2.2](#common-result-meaning), 지속 경고의 발생·해제는 [§3.2.4](#central-warning-rules)를 따른다. 탑승자 존재 관측과 잔류 탑승자 위험은 같은 정보가 아니다.

| 추가 정보 | 논리 표현·용도 | 상태 |
|---|---|---|
| `EVENT_OCCURRENCE_IDENTITY` | 같은 발생의 재전달과 새로운 발생을 구분하는 식별 정보 | 의미 필요, 실제 표현·폭 `TBD` |
| 원 발생 시각 또는 남은 유효기간 | 송신 대기·중계·VSS 대기·출력 준비를 포함한 발생 나이 확인 | 표현 `TBD`, 시간은 [§7.6.3](#vss-event-lifetime)의 `[잠정]` 기준 |
| `INPUT_RECEPTION_QUALITY` | 지속 상태별 `NOT_RECEIVED / VALID / STALE / INVALID` | 내부 구분 필요, 별도 외부 진단은 `OPTIONAL` |
| 기능 가용 상태 | 후방의 유효 DISABLED·UNCONFIRMED와 정상 CLEAR 구분 | [CIS 의미](#sys-cis-s05) 적용, 실제 표현 `TBD` |
...
<a id="if-vss-s03"></a>
#### A.VSS.3 의미 상태와 수신 품질

> **[검토 필요]** 상태·활성 목표의 감시는 SysRS §2.5~2.6의 잠정안을 따른다. 실물 입력·복구·차단 확인과 나머지 경로는 확인이 필요하다.
...
| `SNA / NOT_AVAILABLE` | 정보 제공 기능이 현재 정상 의미 상태를 제공할 수 없다는 표시. 수신 품질과 별도 |
...
후방 거리 측정·필터링·주의/긴급 임계값 판단은 CIS가 담당한다. VSS는 원시 거리 `distance_cm`을 입력으로 요구하지 않으며, 확정된 의미·품질·가용 상태를 사용한다. `CAUTION → EMERGENCY`, `EMERGENCY → CAUTION`, 정상 `CLEAR`는 현재 상태의 변경으로 처리하고 재중재한다. 유효 비활성과 입력을 신뢰할 수 없는 상태는 정상 해제와 구분한다.
...
> **[검토 필요]** 요청 결과 대기는 SysRS §2.3.1의 잠정값을 따른다. 실제 동작 완료·경고 전달 및 인지 기한은 검증이 필요하다.

| 입력 | 전달·복구 규칙 |
|---|---|
| 미시작 단발성 이벤트 | 수용 준비 중이라는 이유만으로 유효한 발생이 무조건 유실되지 않도록 전달 계약을 둠. 원 발생 나이·중복 식별 유지 |
| 만료·중단·완료된 단발성 이벤트 | 기동·복구 후 새 발생처럼 재생하지 않음 |
| 지속 상태 | 과거 이력을 재생하지 않고 현재 유효 의미·품질·가용 상태로 재동기화 |

**[잠정]** 단발성 음향의 실제 시작은 원 발생 후 2초 미만일 때만 허용한다. 선택 시점뿐 아니라 실제 시작 직전에도 확인하며, 발생 나이·남은 유효기간 또는 중복 여부를 확인할 수 없으면 새로 재생하지 않는다. 세부 경계와 이미 시작한 음향의 종료는 [§7.6.3](#vss-event-lifetime)을 따른다.
...
| 음원 자산 사용 불가 | 다른 의미 음향으로 대체하지 않음. 불가 후보를 제외하고 정상 후보 중재 |
...
| 단일 재생·등급별 순서·선점·재중재 | [§7.6.1](#vss-draft-behavior) |
| 유효 해제·비활성·신뢰할 수 없는 입력의 제한 유지와 종료 | [§7.6.2](#vss-input-loss) |
| 단발성 이벤트의 발생 나이·시작·종료 | [§7.6.3](#vss-event-lifetime) |
| 성능의 측정 시작·종료 조건 | [§7.7](#sys-vss-s07) |
| 상대 출력 수준·패턴·시간 후보 | [§7.13](#sys-vss-s13) |

현재 유효 후보에서 하나의 재생 대상을 선택한다. 지속 상태의 변경·재생 종료·선점 해제·고장 복구 시 다시 중재하며, 먼저 수용됐다는 이유만으로 정해진 우선순위를 바꾸지 않는다. 의미와 음원의 대응은 일관되게 관리하고 필요한 자산을 다른 의미의 음향으로 임의 대체하지 않는다.
...
후방 거리 임계값·필터·센서 성능은 CIS의 [§5.6](#sys-cis-s06)·[§5.14](#sys-cis-s14)에서 관리한다. VSS 인터페이스에서 원시 거리나 임계값을 별도로 결정하지 않는다.
