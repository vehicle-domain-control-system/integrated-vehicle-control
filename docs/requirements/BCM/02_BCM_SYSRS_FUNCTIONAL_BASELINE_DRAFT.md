# BCM System Requirement Specification (SysRS)
...
Fan 출력 수준, 온도 장치 방향 및 조명 표시 종류는 **수신한 명령을 보관하기 위한 상태**이며, BCM이 산출하는 값이 아니다.
...
| ID | Requirement |
|---|---|
| <a id="bcm-sys-al-001"></a>BCM-SYS-AL-001 | 수용된 조명 표시 지시를 실제 출력에 반영해야 한다. |
| <a id="bcm-sys-al-002"></a>BCM-SYS-AL-002 | 조명 출력은 지시된 표시 종류와 출력 수준을 모두 반영해야 한다. |
| <a id="bcm-sys-al-003"></a>BCM-SYS-AL-003 | 정의되지 않은 표시 종류를 수신한 경우 직전 출력을 유지하고 오류로 처리해야 한다. *(파생)* |
| <a id="bcm-sys-al-004"></a>BCM-SYS-AL-004 | 현재 적용 중인 표시 종류와 출력 수준을 제공해야 한다. |
...
| <a id="bcm-sys-perf-004"></a>BCM-SYS-PERF-004 | 조명 표시 지시 수용 후 정의된 시간 이내에 출력이 전환되어야 한다. |
...
| <a id="bcm-sys-int-004"></a>BCM-SYS-INT-004 | 조명 표시 지시를 표시 종류와 출력 수준이 구분 가능한 형태로 제공받아야 한다. |
...
| <a id="bcm-sys-int-007"></a>BCM-SYS-INT-007 | 즉시성이 요구되는 조명 표시 지시는 주기 전송을 기다리지 않고 제공받을 수 있어야 한다. |

## 8.2 BCM이 외부에 제공해야 하는 정보
...
| <a id="bcm-sys-int-013"></a>BCM-SYS-INT-013 | 현재 적용 중인 조명 표시 종류와 출력 수준을 제공해야 한다. |
| <a id="bcm-sys-int-014"></a>BCM-SYS-INT-014 | 오류를 센서 오류, 통신 오류 및 기능 오류로 구분하여 제공해야 한다. |
| <a id="bcm-sys-int-015"></a>BCM-SYS-INT-015 | 로컬 안전 판단에 의한 명령 거부를 사유와 함께 제공해야 한다. |
