...
| 영역 | 담당 기능 |
|---|---|
| 중앙 제어 | 차량 수준의 권한·허용·중재, 확정 설정, 자동 기능과 차량 경고 판단 |
| BCM | 도어·실내 공조·실내 조명 실행, 결과 확인과 자체 보호 |
| CIS | 실내 탑승자·환경 관측, 후방 근접 위험 의미와 품질 제공 |
...
이 구분은 논리적 책임이다. 실제 보드 수·물리 배치·통신 경로는 별도로 정한다. 후방 근접 위험의 의미 판단은 CIS가 담당하며, 실행 노드의 자체 보호는 중앙 제어와 독립적으로 동작한다.

```mermaid
flowchart TD
    MOBILE["MOBILE · 요청과 표시"] <-->|"요청·결과·상태"| CENTRAL["중앙 제어 · 권한과 정책"]
    CIS["CIS · 실내·후방 관측"] -->|"관측·품질"| CENTRAL
    TEST["시연 입력 · 잠정"] -->|"사용·이탈·엔진룸·외부 조도"| CENTRAL
...
| 기본 기능 | 목표 온도·자동 공조·선행 공조 설정, 조명 사용·밝기·색상 요청 |
| 기본 기능 | 탑승자 존재/인원수, 온도·습도·실내 조도, 후방 근접 위험과 품질 제공 |
| 지원 구성에 따라 제공 | 단일 창문 채널의 수동·상위 요청, 원터치·위치 이동, 끼임 보호 |
| [잠정] 자동 기능 | 선행 공조·자동 환기는 [§11.4](#sr-auto-profile)의 기본안 적용. 실제 허용·피드백 확인 필요 |
| [잠정] 경고 조건 | 잔류 탑승자·끼임·이탈 후 열린 도어·엔진룸 동물 경고는 [§11.3](#sr-warning-rules)의 조건 적용 |
| [잠정] 외부 조명 구성 | 대표 1채널 ON/OFF·임시 점등 지원, 밝기 수준 조절·실제 점등 여부를 확인하는 피드백은 기본 구성에서 미지원 |
| 기본 모바일 범위 | 활성 화면의 인증된 연결에서 제어·즉시 경고 제공. 창문은 상태 표시 대상 |
| 선택 기능 | 모바일 백그라운드 연결·즉시 알림, 후방 감지의 후진 기어 연계 |
| [잠정] 시연 입력 | 차량 사용·사용자 이탈·엔진룸 감지·외부 조도는 별도 시연 입력으로 연결 가능. 실제 센서 구성과 외부 자동 점등 조건은 확인 필요 |
| 현재 제어 범위 밖 | 모바일의 창문·외부 조명 원격 제어 |
| 영역별 제외 | CIS의 엔진룸 감지·액추에이터 구동·음향 재생, EXTERIOR_LIGHT의 외부 조도 직접 측정·자동 정책 판단 |
...
창문 끝단 전이 식별 요구는 유지한다. 실제 정보 사용 기능과 전달 형식은 미정이다. VSS 보안 확장은 현재 기능 범위에 포함하지 않는 후속 후보이다.
...
| 용어 | 의미 |
|---|---|
| 중앙 제어 | 차량 수준의 권한·허용·중재·자동 정책을 담당하는 논리 기능. 상위 시스템·중앙처리장치·Domain은 이 기능의 별칭 |
...
| 품질 / 최신성 / 인증 | 정보의 사용 가능성 / 정보가 얼마나 오래되었는지 / 출처·권한의 신뢰 |
| 존재 / 위험 | 관측한 탑승자 존재 / 차량 조건을 함께 판단한 위험 상태 |
| 자체 보호 | 실행 노드가 직접 확인한 조건에 따른 안전 정지·출력 제한 |
| 공조 / 선행 공조 | 실내 공기 순환·냉각·가열 / 예정 시각에 앞서 수행하는 공조 |
...
- <a id="sr-bcm-l050"></a>시스템은 현재 잠금 상태와 동일한 목표의 요청이 반복되더라도 반복 동작이 발생하지 않도록 해야 한다.
...
> **[검토 필요]** 선행 공조는 [§11.4](#sr-auto-profile)의 잠정 예약 조건을 따른다. 실제 시각·구동 허용·방열 피드백 검증은 필요하다.

- <a id="sr-bcm-climate"></a>자동 온도 조절·수동 공기 순환·선행 공조가 겹칠 때는 하나의 공조 동작을 선택해야 하며, 대체·정지된 기능이 이전 설정만으로 다시 시작되지 않아야 한다.
...
> **[검토 필요]** 실내 조명의 지원 색상·패턴과 밝기 표현은 미정이다.
...
> **[검토 필요]** 보호 해제와 재동작의 실제 확인 방법·시간은 미정이다.
...
- 시스템은 오류의 복구 조건이 충족되기 전에는 정상 동작을 재개하지 않아야 한다. 팬은 정지 상태의 기본 허용을 확인한 뒤 새 요청에 따른 제한된 복구 확인만 수행할 수 있어야 한다.
...
본 절은 차량 실내의 탑승자 상태와 환경, 그리고 후방 근접 위험을 감지하여
...
판정 결과는 중앙 제어와 차량 음향 시스템(VSS)이 각각 필요로 하는 형태로 제공되어야 한다.
...
> **[검토 필요]** 경고 조건과 시연 입력은 [§11.3](#sr-warning-rules)을 따른다. 실제 감지 장치·입력 신뢰성·전달 기한은 확인이 필요하다.

- CIS는 엔진룸 대상 동물의 진입 여부를 판정하지 않는다.
- CIS는 후방 위험 상태에 대응하는 음향을 직접 재생하지 않는다.
- CIS는 도어 잠금, 파워윈도우 등 차량 액추에이터를 직접 제어하지 않는다.
- CIS는 확정된 판정 결과와 측정값을 중앙 제어 및 VSS가 활용할 수 있도록 제공해야 한다.

<a id="sr-cis-s04"></a>

### 6.4 탑승자 인식 요구사항

> **[검토 필요]** 탑승자 존재와 잔류 위험은 구분한다. 잠정 경고 조건은 [§11.3](#sr-warning-rules)을 따르며 실제 관측·차량 입력의 신뢰성은 확인이 필요하다.
...
> **[검토 필요]** 외부 자동 점등·ON/OFF·10초 임시 점등은 [§11.4](#sr-auto-profile)의 잠정안을 따른다. 실제 조도 센서·부품·보호·물리 배치는 확인이 필요하다.
...
- <a id="sr-cis-l088"></a>시스템은 물체와의 거리가 정의된 기준 이내인 경우 근접 위험 상태를 판단해야 한다.
- <a id="sr-cis-l089"></a>시스템은 근접 위험 수준이 높아지는 경우 이를 구분되는 상태로 제공해야 한다.
- <a id="sr-cis-l090"></a>시스템은 후방 감지가 활성인 동안 유효한 거리 정보로 위험 기준 밖임이 확인되거나 정상적인 미감지가 확인된 경우 근접 위험 상태를 해제해야 한다. 센서 무응답이나 감지 비활성화를 위험 해제로 처리해서는 안 된다.
- <a id="sr-cis-l091"></a>시스템은 후방 정보의 신뢰성이나 활성 조건을 확인할 수 없으면 위험 수준과 위험 해제를 확정하지 않아야 하며, 확인 불가 상태를 구분하여 제공해야 한다.

- <a id="sr-cis-activation"></a>현재 기본 구성에서는 유효한 차량 전원 허용 상태에서 후방 감지를 제공해야 한다. 후진 여부에 따른 제한은 선택 구성으로 구분하고, 감지 비활성 및 활성 조건 확인 불가를 정상 위험 해제와 구분해야 한다.
...
### 6.8 중앙 제어 및 VSS 연계 요구사항

> **[검토 필요]** 지정 상태의 주기·최신성은 [SysRS §2.5](sysRS.md#common-freshness-profile)의 잠정안을 따른다. 실제 전달 방식과 충족 여부는 확인이 필요하다.

- 시스템은 유효한 탑승자 판정 결과 및 환경 측정값을 중앙 제어로 전송해야 한다.
...
CIS의 핵심 기능은 **실내 탑승자·환경 상태를 판정하고, 후방 근접 위험을 주의·긴급·해제 상태로 구분하여 제공하는 것**으로 한정한다.
...
> 중앙 제어와 VSS가 각각 필요로 하는 형태로 제공하는 실내 센싱 기능이다.**
...
- <a id="sr-mobile-l047"></a>최근 제어 요청의 수행 결과를 사용자가 확인할 수 있어야 한다.
...
> **[검토 필요]** 인증 방식·키 관리와 차량 측 권한 폐기 절차는 미정이다.

- <a id="sr-mobile-l120"></a>인증 및 차량별 권한 확인이 완료된 사용자에게만 원격 차량 제어 기능이 제공되어야 한다.
- 차량에서 온 정보임을 확인할 수 없는 경우 새로운 차량 제어 요청이 생성되어서는 안 된다.
- 같은 제어 요청이 두 번 전달되거나 늦게 전달되더라도 차량에서 반복 수행되지 않아야 한다.
- <a id="sr-mobile-l123"></a>인증 정보는 사용자가 직접 열람하거나 입력하지 않아야 한다.
- <a id="sr-mobile-unregister"></a>단말에서 차량 등록을 해제하면 해당 인증 정보를 삭제하고 연결·새 요청을 중단해야 한다. 단말 삭제와 차량 측 권한 폐기는 구분되어야 한다.
...
- <a id="sr-mobile-l153"></a>사용자 인증의 판단은 차량 측에서 수행되어야 한다.
...
- 유효한 요청의 실행으로 도어가 목표 잠금 상태로 실제 전이하여 정상 완료된 경우 잠금 완료를 인지할 수 있는 음향이 제공되어야 한다. 이미 잠겨 있는 정상 도어의 무구동 완료에는 새 완료음을 제공하지 않아야 한다.
- 유효한 요청의 실행으로 도어가 목표 잠금 해제 상태로 실제 전이하여 정상 완료된 경우 잠금 완료와 구분 가능한 음향이 제공되어야 한다. 이미 해제된 정상 도어의 무구동 완료에는 새 완료음을 제공하지 않아야 한다.
- 수용된 잠금 요청의 실제 실행 후 목표 잠금 상태 확인에 실패한 경우 정상 잠금 완료음과 구분 가능한 경고가 제공되어야 한다. 실행 전 거부 또는 결과 확인 불가만으로 잠금 실패 경고를 발생시켜서는 안 된다.

<a id="sr-vss-s05"></a>

### 8.5 안전 및 주의 경고 요구사항

> **[검토 필요]** 끼임 경고는 [§11.3](#sr-warning-rules)의 잠정 현장 확인 조건으로 해제한다. 확인 입력의 구성은 미정이며 반전 완료·STOP만으로 해제하지 않는다.
...
> **[검토 필요]** 원터치 입력 구분·자동 환기는 [§11.4](#sr-auto-profile)의 잠정 기본안을 따른다. 스위치 부품·위치 보정·반전 목표/시간은 미정이다.
...
> **[검토 필요]** 기본 외부 조명은 실제 점등 여부를 확인하는 피드백 미지원으로 잠정 정한다. 출력 적용 확인 방법·기한과 보호 회로는 확인이 필요하다.
...
- 원격 제어는 인증된 사용자의 대상 차량·기능 권한에 따라 제공되어야 한다. 차량 출처 확인 실패와 개별 센서의 품질 저하를 구분하고, 관련 없는 품질 저하 때문에 사용 가능한 모든 제어가 차단되어서는 안 된다. 기능에 필요한 정보와 로컬 보호 조건은 유지되어야 한다.
- 이미 목표 잠금 상태인 정상 도어는 추가 구동 없이 요청을 완료해야 한다. 이 완료는 새로운 잠금·해제 발생이나 새 완료음으로 취급하지 않아야 한다.
...
- **[잠정]** 차량 사용·사용자 이탈·엔진룸 감지·외부 조도의 실물 입력이 준비되지 않은 시연에서는 별도 시험 입력을 사용할 수 있어야 한다. 모의 입력과 그 경고는 시연임을 식별할 수 있어야 하며, 실제 감지가 제공된 것으로 표시해서는 안 된다.
...
| **[잠정]** 엔진룸 동물 | 별도 엔진룸 감지 입력에서 동물 존재가 유효하게 확인됨 | 동일 감지 영역에서 동물이 없다는 새 유효 관측이 확인됨 |

잔류 탑승자 경고는 사용 종료 후 남은 탑승자를 알리는 시연 기준이다. 온도에 따른 신체 위험 수준을 판정하는 기준은 아니다. CIS의 탑승자 관측과 엔진룸 감지는 별개다.
...
| 기능 | 잠정 기본 요구 |
|---|---|
| 선행 공조 | **[잠정]** 예약 1건을 1회 실행한다. 탑승 예정 시각 5분 전에 시작하고 예정 시각 또는 최대 5분에 도달하면 종료해야 한다. 시작 조건 미충족·정지·실패 후 자동 재시작하지 않아야 한다. |
...
자동 기능은 실제 구동 허용과 기능별 피드백·로컬 보호 조건이 확인된 경우에만 실행해야 한다. 사용자 수동 조작·정지·허용 상실 시 해당 자동 작업을 끝내고 이전 작업을 임의 재개해서는 안 된다. 예약이나 목표 적용 완료를 실제 온도 도달·창문 위치·실제 점등 여부를 확인한 결과로 대체해서는 안 된다.

선행 공조와 자동 환기는 함께 실행하지 않는다. 자동 환기는 현장에서 새로 사용을 허용할 때마다 한 번만 시작하며 모바일에 환기 제어 기능을 추가하지 않는다. 입력 구성과 세부 종료 조건은 [SysRS 자동 기능](sysRS.md#central-preclimate), [지원 구성](sysRS.md#demo-capabilities)을 따른다.
...
**[잠정]** 늦게 도착해 수용 기한을 지난 새 요청은 실행하지 않아야 한다. 실행 중인 상위 목표는 현재 유지 허용을 확인하며, 허용 상실 시 해당 기능의 정지·소등 규칙을 적용해야 한다. 오래된 요청이나 유지 허용의 재전달만으로 동작을 연장하거나 재개해서는 안 된다. 적용 시간과 로컬 보호의 구분은 [SysRS 명령 수명](sysRS.md#common-command-lifetime)을 따른다.

**[잠정]** 단발성 음향은 원 발생 후 2초 미만일 때만 새 재생을 시작해야 한다. 경고 해제·유효 비활성 또는 신뢰할 수 없는 입력에 대한 제한 유지 종료로 중단하는 음향은 100ms 이내에 해당 출력을 종료해야 한다. 음향 종료와 위험 해제는 구분해야 한다.
