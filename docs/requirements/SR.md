...
- 시스템은 실내 영상을 이용하여 탑승자 인원수를 판정해야 하며, 판정 가능한 인원수 범위는 `0 ~ 5명`으로 한다.

**판정 유효성·비전 복구**

- 시스템은 탑승자 판정 결과의 유효 여부를 구분해야 한다.
- 시스템은 탑승자 존재 여부 판정과 인원수 판정 결과가 동일한 판정 회차의 일관성을 갖도록 제공해야 하며, 부재(`ABSENT`) 상태와 1명 이상의 인원수를 같은 판정 회차의 정상 결과로 제공하지 않아야 한다.
- <a id="sr-cis-l067"></a>시스템은 비전 정보를 신뢰할 수 있기 전에는 해당 정보에 기반한 판정 결과를 확정하지 않아야 하며, 비전 오류나 인식 불가를 탑승자 부재로 대체하지 않아야 한다.
...
> **[검토 필요]** 초음파 센서는 `HC-SR04`(물리 측정 범위 2~500 cm)를 사용하며, 단위는 cm로 측정한다.
>
> - 유효 관측 범위(후보: 10 ≤ d ≤ 100 cm) 및 정확도, 차량에서 감지를 허용하는 입력, 측정부터 전달까지 걸리는 시간도 벤치에서 시험해야 한다.
> - 주의·긴급을 나누는 거리 기준은 중앙 제어가 관리한다.

**거리 측정·유효성·중앙 제공**

- <a id="sr-cis-l086"></a>시스템은 초음파 센서를 이용하여 후방 물체와의 거리를 cm 단위로 측정해야 한다.
- <a id="sr-cis-l087"></a>시스템은 거리 측정 정보의 유효 여부를 구분해야 한다.
- <a id="sr-cis-distance-output"></a>CIS는 측정한 거리와 감지 상태(`PROXIMITY_STATUS`), 유효성, 측정 시점과 새 측정 여부를 중앙 제어에 함께 제공해야 한다.
- CIS는 정상적으로 물체가 없음을 확인한 경우(`NO_OBJECT`)와 센서 무응답·범위 밖·측정 실패(`UNAVAILABLE`)를 명확히 구분해야 한다.
- 실패한 측정에 임의의 거리값을 넣어 정상 정보로 제공하거나 안전 상태로 대체해서는 안 된다.

**기본 활성과 선택 후진 구성**

- <a id="sr-cis-activation"></a>현재 기본 구성에서는 유효한 차량 전원 허용 상태에서 후방 감지를 제공해야 한다.
- 후진 여부에 따른 제한은 선택 구성으로 구분하고, 감지 비활성 및 활성 조건 확인 불가를 정상 위험 해제와 구분해야 한다.
...
- 시스템은 판정 결과와 측정값을 제공할 때 각 값의 유효 여부(`VALIDITY`)와, 유효하지 않은 경우 그 사유(`QUALITY_REASON`)도 함께 제공해야 한다.
- 시스템은 각 관측값의 원본 생성 시점 또는 경과 시간(`SOURCE_TIMESTAMP`/`AGE`)과 갱신 식별자(`UPDATE_SEQUENCE`)를 함께 제공하여 중앙 제어가 새 관측값과 과거 값의 단순 재전달을 구분할 수 있도록 해야 한다.
- 시스템은 전체 ECU 상태 외에도 기능별 상태(`FUNCTION_STATUS`) 및 제공 경로 상태(`INTERFACE_STATUS`)를 제공해야 한다.
