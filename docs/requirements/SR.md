...
- 시스템은 근접 위험 상태를 VSS가 활용할 수 있도록 주의 / 긴급 / 해제로 구분한 위험 상태로 제공해야 한다.
...
- 후방 장애물의 실제 거리값 및 임계값을 VSS 등 외부에 직접 노출하는 것 (판단한 위험 상태만 제공)
...
- CIS의 후방 위험 상태는 VSS가 사용하는 위험 단계·상태 이름과 일치해야 한다.
...
> 후방 물체와의 근접 위험을 주의/긴급/해제 상태로 확정하여,
